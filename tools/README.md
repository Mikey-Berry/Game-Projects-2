...
| `discourse.js` | What the world has heard, and what it says to you about it. **Fame is two numbers and that is the design**: `known` only rises, because a story does not unhappen, and `repute` moves both ways — the BAND comes off known and only the WORD comes off repute, so famous-and-loathed is a place on the scale rather than a cancellation. Asserted by holding known at 70 and flipping repute: same rung, "notorious" against "renowned". The first version of that test ran twelve conquests against twelve towns saved and compared bands, but a conquest is worth 8 known and a saved town 7, so the arms landed on different RUNGS and it reported a design failure that was really a difference in deed weights. The dialogue half is asserted through the **rendered buttons**, never off the tree data — every gated option existed in the table before any of it was wired to `talkTo`, so "the option exists" proves nothing; what the report asks is that the right person sees it. Its own care: the fame sections cannot degrade (there is no earlier version of a thing that does not exist) but the dialogue sections can and do, so the negative control reports no modal and empty menus rather than one ReferenceError. |
