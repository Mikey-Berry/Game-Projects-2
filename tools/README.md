...
| `cold.js` | What a storey nobody is on is allowed to cost, and the three ways making it cheap could go wrong quietly. The premise is counted before anything is asserted, because if it stops being true the tier should go: of 1,748 bodies, **1,133 on a storey with no player unit on it, and only 166 of them idle** — twelve hundred bodies walking and fighting four storeys under somebody who cannot see any of it, which the old `_far` tier could not touch because it only drops a body with no quarry. So the gate is the STOREY, and two claims pin the edges as exact zeroes: nothing of yours is ever cold, and nothing on a floor one of yours is standing on is either. One body walked down to -2 wakes all 326 of them and leaves -3 asleep — per-floor, not "underground is awake". Then `stepToward`, which tested collision at the destination and nowhere between: exact while every step was a sixth of a tile, and a hole in the world the moment one is a tile and a half. It is driven at 1/30s, 0.3s, 1s and **3s** — far longer than the tier will ever hand it — because a bound that only holds at the rate it was tuned for is not a bound. **Section 4 is the one this file exists for.** `hostile` is memoised on a key built from every field the rule reads through four helper functions, and a key that misses one makes two bodies interchangeable that are not — silently, and in the direction of things NOT fighting that should. So it is checked rather than argued: ~15,500 real pairs from the live world, both orders round, memo against raw, **with the hostile count asserted non-trivial** because a comparison where every answer is `false` agrees perfectly and proves nothing. Then the two specific collisions a sample might simply not contain — a concealed risen against a bare one, a provoked Kept against a calm one — since those are the ones that would cost you a fight. Four of its own faults are written into it: `_lod` and `_cold` were conflated, which broke every fight underground (`separate` is where `_contacts` is written, and it skips pairs where both are `_lod`); the arrival claim shoved a `moveTarget` into a gaunt, which is a field the body's own ai owns and cleared within the second, so it measured the ai and reported 2.2 tiles of 486 while `physics` ran at exactly the right rate; the fight claim staged one pair and asked whether either died, which is far too noisy — the same pair on a warm floor came back at 72, 85 and 86 blood on three runs and never killed anybody either, so its red line was describing variance; and the wall claim searched one room's north ring, which is where the approach tunnel usually arrives. |
...
- **A cache key must carry every input, and the proof is a comparison, not an argument.** The
  enmity rule reads seventeen per-body fields through four helper functions, one of which is
  position-dependent and one of which calls the rule back. A faction-pair cache would have been
  wrong silently and in the direction of things not fighting that should. The claim that makes it
