...
| `review.js` | **The ten bugs the 2026-09-24 line-by-line review found, each staged the way a player meets it.** Quadrupeds drawn at the map's north edge (it measures the mesh, not the body, because the body was always in the right place); a built wayline that right-click could not open; host upkeep and the Binding Circle both blind to remains in a Reliquary; the wagon's EAT feeding a risen; tribute and penalties alike pulling a hated town up to -100; a pauper spawner leaving corpses off the roster; a window resize throwing away the PIXEL RATIO cap (so it runs on a 2x context); ctrl+right-click ASSIGN putting a mindless risen on CRAFT; and the Aldercott deal not surviving a save. Ten reds on 30223c9, ten greens after. CODE-AUDIT.md §1 has both columns verbatim. |
...
| `sigcost.js` | What a frame spends **deciding whether to rebuild**. `render()` calls seventeen `sync*` functions every frame and each builds a signature string; nothing else here timed them. Interleaved A/B, per-signature medians, with the sim step beside it as a control: `node tools/sigcost.js prev.html game.html [rounds] [steps]`. `syncRedoubts` was 1.97 ms of 2.19 before 2026-09-24. |
