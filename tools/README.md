...
| `command.js` | Whether a band you sent away actually goes, does the thing, and comes back. A commander is made almost entirely of other features — it writes no movement, no fighting and no looting, it points the existing per-unit order fields (`attackMove`, `moveTarget`, `chestTarget`, `lootTarget`, `guardTarget`) at things — which makes it very easy to build something that reads correctly, syntax-checks, and does nothing at all in play. So this runs the real sim forward and looks at where the bodies ended up: did they leave, did they walk a circuit rather than a line, did the band keep up, did they open the chests inside the order's ground and leave the one outside it, did they come home when it was clear, does a captain who falls release his band, does the order survive a save — and does the band re-form after one, since `guardTarget` is an object reference and cannot. It also proves the break-off judgement differs by captain: compassion turns back at two down of four, ambition presses on to three; that a standing duty walks tour after tour on its own and will not begin one after dark; that a surgeon in the band both works in the field and moves the captain's break-off line; that a mule doubles what a sweep can carry; and that a contact call carries a count, a bearing and reaches the world news exactly once per contact. Three things it found that reading the source did not: a patrol had no END of tour, so a band that met nothing simply never came home and "standing duty" had nothing to stand between; a captain killed on a posted duty released his band in total silence; and the probe's own teardown used `chars.splice(chars.indexOf(o), 1)`, which on a body no longer in `chars` is `splice(-1, 1)` and deletes the last character instead — five of those took the player's own body out from under the next two blocks. **AND IT WAS GREEN FOR THE WRONG REASON, which is the third instance of that this repo has written down.** Its forage claim placed chests with `findOpenNear`, which returns an OPEN tile — and an open tile inside a sealed pocket is open and *unreachable*. A* returned no path on **every** build ever run, the passing ones included. The band got there anyway, because before `stepToward` bounded a long step it sampled the destination and nothing between, and this harness steps at **dt = 0.25**, where the stride clears a one-tile wall: the claim was measuring the tunnelling leak that bounding was added to close (`cold.js`'s `noStepWalksThroughRock` is red on 902efd0 at dt = 1 and dt = 3). Closing the hole turned the claim red, and the claim was the thing that was wrong. **Underneath it sat a real and pre-existing bug**, found with `tools/_forage.js`: a captain at (1053.12, 565.72) with a chest at (1050.66, 565.72) — identical y, wall due west — held the errand and did not move a ten-thousandth of a tile for 116 sweeps, then starved standing there. `stepToward`'s two wall-slide branches try each axis alone, and on an exactly axis-aligned target one of those points **is the body's own position**, which `isBlocked3` reports as clear: it assigns `c.y` the value it already held, records that the body moved, and breaks. `travelStuck` will not detour inside four tiles (deliberately), and `commandTick` waits on an errand forever. Fixed three ways — a slide must displace, errands give up on what they cannot reach (patience measured in **ground gained**, not time spent, so a body in a doorway or a crowd is never abandoned), and the harness now asks A* whether its chests are reachable. `givesUpOnTheUnreachable` locks it in, and was checked **red on the pre-fix build** — "120 sweeps, 1.5 tiles off, captain down" — because a claim that passes on both builds proves nothing. |
...
| `tongue.js` | Whether a tongue is worth anything, and whether the gate is awake. **Two racial flags had an economy and nobody else had a lever**: a scaleborn at a counter put prices up a flat 15%, a Fallen put them down a flat 10%, and every other body in the game paid list forever. Both are charisma now, and the claims are that the curve saturates in both directions (it costs you money at the bottom, pays at the top, and never reaches free — a stat that runs linearly to a 100% discount is an exploit), that it moves BOTH sides of the counter, and that it is trained by gold moved rather than by clicks: two hundred two-gold deals must not out-train four real ones, which is the law of the shovel that `labor` learned the hard way. **And the second half started as a measurement.** Standing in the middle of Dustport for five game days with a proscribed book in hand produced ZERO stops on the old build — the search wanted a town guard within six tiles and the nearest posted guard to that town's centre is 9.7. At the gate it is 1.7. The mechanic already lived at the gate and nothing arranged for it to happen there, so a search is a WALL CROSSING now and forty crossings give eighteen stops against the old build's two. Its own two traps, both recorded in the file: the route claim was written first as the better game — the gate is watched, the far wall is not, sneak round — and measured false, because eighteen guards ring Dustport at radius 19-25 and the worst gap in twelve samples is seven tiles; and the file reported the gate asleep for two whole claims because **a harness that pauses at boot is running a world in which nothing derived from a tick has ever been computed**, so `charsNear` was reading an empty char grid and found no guard standing a tile and a half away. |
| `parley.js` | Who you can talk to, what they say back, and what a held click offers instead. **`c.guard` holds a POST — `{x, y}` — not a rank**, and is set on cave dwellers, wyrm keepers, reclusive scholars, the guildmaster and the Watcher that moves into the Bastion yard after day 45, which is the gaunt in the report that opened this file. Five other reads in the game pair it with `faction === 'town'`; the two that did not were the conversation router and its title line, and the unreported consequence was worse than the reported one — every `npcNecro` carries a post too, the guard test ran first, and the necromancer tree shipped the round before was unreachable by anybody in play. It shipped green because `discourse.js` staged a necromancer with no post; stage the body the world actually makes, or the claim is about a body that does not exist. Also asserts the spoken line's CONTRAST as a number (`--ink` in this palette is the darkest background, not the text: the broken build measured 1.09:1, which is not dim, it is the same colour twice), that asking for news never returns ambient chatter while the barks still exist under their own name, that a held right-click offers a menu and that guarding a non-squad NPC takes, and that round-tripping one stack at a vendor teaches 0.29 charisma per hundred clicks where it used to teach 25.6. Its own trap: `shut()` hid the modal without emptying it, so a correct silence read as the previous probe's buttons. |
| `upkeep.js` | Five small things that were wrong in play, four of them one line each, and every one a DEFAULT nobody chose: a camera fallback (`|| player()[0]`, which slid the view 311 tiles home the moment you cleared the selection), a missing early return (a gravecart with no yard took a corpse, found nowhere to put it, dropped it, and picked it up again), a missing filter (a standing BUILD order had no range at all on a 1440-tile map), a ceiling with nowhere to be read (a binder found out they were full by walking to a circle and being refused), and a one-hour revive window in a game where an hour is eight real seconds. **Two of its five claims passed on the broken build until the control was run**: the camera claim re-implemented the camera line inside the probe and asserted against its own arithmetic, and the revive claim asked `decayStage` — the NEW rule — instead of casting the spell. A probe that restates the rule it is testing is not a test. Both drive the real thing now, `update(1/60)` and `resolveCast`. Two further faults were staging rather than logic: a corpse staged inside a town's exclusion radius, which `cartFodder` is right to refuse, and four corpses stacked on one tile where the cast only ever reaches the first. |
| `orders2.js` | One book, read by every hand — and the smelter that could not be used. **It was never a bug in the smelter.** The whole CRAFT job was eleven lines that walked to a `workbench` and made build materials; `jobHasWork` agreed (`b.type === 'workbench' || b.type === 'forge'`), and the kiln, the loom and the smelter were in neither that list nor `BUILD_JOB` — so right-clicking a smelter never offered the job that works one, and everything those three buildings make could only be made by opening their own window with a body standing inside it. One cause, two reports: the broken smelter and the request for a universal crafting menu. Asserts that a camp whose only workshop is a smelter counts CRAFT as work, that the book lists all five workshops and says why a dead row is dead, that an order for ingots sends its hand to the SMELTER rather than the bench thirty tiles away, and that an order of three makes three and then takes itself off the book. Its own trap: the probe guessed `canDoJob` for a predicate actually named `jobHasWork`, and the `typeof` guard turned that into a claim reporting `null` rather than throwing — a claim can measure nothing at all and still look like it ran. |
| `kitdoll.js` | The kit window: every slot reachable from the pack, and a figure that is the actual body. Two reports, one screen. The backpack row was hand-written against `weapon\|armor\|pack` — the three slots that existed before head, cloak and trinket were added — so **a cloak in your own pack offered GIVE, STASH and DROP and nothing else**, which is the report verbatim and what the control prints. It also found a second bug on the way past: that path never asked `gearRefusal`, so the pack was a way round rules the shelf enforces, and a gifted body could take up the Aether Lance out of its own backpack. And the paperdoll figure was an SVG of six tinted regions — the same drawing for a Choir Kin, a golem and a Sixfold — now `buildCharMesh` in a second small WebGL context. Asserts the verb exists for all six slots, that pressing it moves the item, that refusals still bite, that the figure is a canvas rather than an SVG, **that it actually draws** (36% of the panel non-transparent), that it is the real rig, that it turns, that stripping gear changes the vertex count, and that fourteen redraws leave exactly one canvas with the world still drawing — because a browser hands out about sixteen WebGL contexts and then takes the oldest back, which would be the main renderer. Its own trap is in the file: it first read the portrait with `drawImage` and counted **0 of 93,984 pixels lit** off a portrait that was visibly drawing in a screenshot, because a WebGL drawing buffer is cleared after compositing and `preserveDrawingBuffer` is off by default. Render and copy in the same task. |
| `legends.js` | The first twelve legendaries, and the standard the doc set for them: **vanity changes nothing, ordinary gear moves numbers, a legendary changes exactly one rule**. So no claim here asks whether an item exists or whether a flag is set — every one takes a live body, reads the rule with the item off, puts it on, and reads it again. The Stave takes the binding ceiling 8→13; the Weave takes 45% of strain off a nine-bound caster to zero; plate spreads 21.7 across cut/pierce/blunt/burn and **the Eleventh Coat spreads 0**; the Habit unhallows the ground *for raising only*, and the ground is still consecrated afterwards, because `consecratedAt` is read in five places and only three of them are the rule; the Circlet takes 2/2/3/2 to 3/3/3/3 and still leaves a thing bound around one formula with room for exactly that one; the Shroud hides a risen six tiles from the bearer and not one thirty tiles off; the Stridercloak carries plate and an armet at a duster's 3.81; the Mantle makes a stalker take the man five tiles behind you **and still take you when there is nobody else in the room**; the Splinter is free with a tear six tiles away, not free with the same tear two hundred tiles away, and costs its 6 with it closed. The Tooth gets two claims because it is the only one that needed a mechanism rather than a flag: a bandage cannot close it, `treatable` says so before a medic walks over, four hours does not clot it, twelve takes the arm, and a divine hand closes it — after which cloth works again. Then the doc's placement rule, counted rather than trusted: exactly one of each in the world, none in an ordinary chest, none on any vendor's list, none in any of the seven towns' books, and the Armoury given a Stave, a Weave, Sal Mortis and one katana hands over **the katana**. Two of its own claims were wrong in ways worth keeping: the Coat claim built a fresh body per damage type and measured the random **subrace's** `vuln` row rather than the coat, and the save claim mutated the body after `snapshot()` — which holds `parts` by reference — and so measured a save that never held the value. |
| `deep.js` | The Kept — humanity's own progenitors, still keeping a vigil for a god the Golden Age turned into plumbing — plus the Stonewake and the Shoal. Sixteen claims, and the control prints **NOTHING TO MEASURE** sixteen times because none of it existed. Asserts the four kinds are seeded under the world at kept altars and carrying bronze; that the enmity with the gaunts holds *both ways round* off real bodies, and that neither the Kept nor a shoalling will raise a hand to you until you start it (and that provoking is reversible). Then the characterisation, driven through the real `ai` at the real rate: ten seconds and five of them never come closer than eight tiles, all five turned to face — and a lamp moves them from eight tiles out to eleven, **driven rather than burned**, which is the inverse of every other rule about light in this game. The Unblinded is the exception and is measured as one. The offering is checked for the *shape* of its answer rather than a number — bones buy bronze, a written thing buys insight, a charge cell is put out and answered with a cloak, and five hundred in coin changes their regard by exactly zero — and enough of it turns the vigil's back. One blow through `applyDamage` turns all of that hall and **none of the 178 elsewhere**. The Stonewake opens 19 tiles of solid rock and drives a lane from 22 blocked tiles to 8, on a budget it cannot exceed. Its own traps are in the file three times over: it measured a congregation that had correctly walked off to kill a Thin Stalker (twice, because a Kept looks twenty-two tiles from where it is *standing*), it counted rock in a six-tile radius **around a body that moves**, and it fed fruit to a row that wanted meat. **AND THE LAMP CLAIM COULD NOT TELL THE LIGHT FROM THE BAND.** `andALightPushesThem` rang twelve bodies at eight tiles, lit a lamp, and asserted they ended two tiles further out — but they drift to the edge of their own hold band with or without a lamp, so "8 to 11" was never evidence. It is an A/B from one staging now: **dark they sit at a mean 8.0, lit at 10.6.** Three faults were fixed getting there. The ring was drawn WITHOUT LOOKING AT THE ROCK, so on a tight hall half of it landed inside stone and `physics` pushed those bodies back toward the middle — the claim read 8 then 5.2, a congregation apparently walking INTO the light. The replacement A/B lit the lamp AFTER `rebuildCharGrid`, and `lamps` is built BY that rebuild off `c.lamp`, so both halves came back reading exactly the staging distance. And the verdict reads the MEAN, because the closest of twelve is one body and one body with rock behind it cannot give ground however bright the lamp is — it moves 0.5 tiles, and is kept only as a guard that nobody is drawn IN. | |
| `dark.js` | The undercroft made dark, and what that costs the people standing in it. Sixteen claims; the control against HEAD prints **NOTHING TO MEASURE** ten times and fails the other six on the mechanics that did exist — including the standing bug it was written to catch: on the old build a **sealed tunnel was seventeen tiles at midnight and twenty-nine at noon**, because `computeVision` was reading the surface day curve twenty-two units underground. The ladder is measured off `stampVision` rather than off a flood fill of the fog sheet, because the claim is about what the code chooses: eleven blind, eighteen for something that needs no light, twenty-two with a torch, against thirty on the surface — a room, deliberately, not a letterbox. The exemption is asked of a real risen, a real gaunt, a real Kept and a real `caveDweller` rather than of the flag table, since `seesInDark` reads four marks and a body can carry any one. The penalties are all end-to-end: the same evenly-matched pair swings lit and unlit for an **exact** twenty points of hit chance and thirty-four for a bow (the first version gave the attacker twenty-four attack against ten defence, every lit reading came back at the 0.94 clamp, and the ranged claim went red on a mechanic that was working); two identical bodies work the same seam through thirty seconds of real `physics` for 40 units against 10; a builder raises 98% of a frame beside a burning post and **exactly nothing** without one. Then the torch as an object: six hours of daylight takes nothing off it and six hours below takes six, two spares in the pack are drawn and lit without a click and the hand comes out empty, a post pulls one log a day out of the stores and goes dark when they are empty, and a Storage Bin staked underground is refused rather than silently raised on the hillside above. Two of its own fixtures were wrong in ways worth keeping: the build claim measured a builder who correctly went to fight a stalker an earlier section had spawned, and the post claim doused four leftover posts instead of removing them, so they ate the woodpile before the one under test got a look at it. It also holds the general form of the exemption — **every hostile body under the world either sees down there or is carrying a light** — which caught a regression on the way in: dropping `caveDweller` from `seesInDark` (right, since it means "this AI lives in this warren" and worldgen puts it on the bandits too) silently blinded the whole redoubt garrison. And it prints one **NOTE** it deliberately does not assert on, because the bug is not this feature's: there is not one bandit under the world. `seedWarrens` assigns `cv.menace` *after* `placeCave` runs the stocking, so `d01` falls to its 0.5 default and **all three warren bands collapse to the middle one** — 447 gaunts, no bandits, and the "shallow = bandits, deep = the worst" layering has never once run. Fixing it moves 447 spawns and shifts the world's random stream, so it is recorded rather than bundled. **AND IT WAS STAGED UNDER THE SEA.** It took `undercroft.halls[0]`, and several claims stake or walk on the SURFACE above that hall — but the undercroft runs the full width of a world that now has an ocean over its rim, so `halls[0]` can be, and on the default seed is, under water. `onlyTheFireGoesDown` came back with every value null and read as the game refusing to build anything, which it was right to do. It picks the first hall with open ground over it now, and the stakes go on tiles checked to be free rather than at a blind `hall + 5`. | |
| `seats.js` | Two towns built around the thing they are about. The pit and the Aldercott estate were each seeded by walking a ring *outward* from the wall until they found ground nobody was using — which is how you place a thing that has nothing to do with the town it is next to, and what the control prints: **the pit 39 tiles outside a wall of 26, the estate 44 outside a wall of 24, and ten buildings per town standing where the plaza should be.** Asserts both are now on their town's own centre and inside its wall, that no building is left in either reservation, that the Aldercott house has three storeys of *walkable deck* with a real stair on each (a renderer can draw three storeys over a building nobody can go up in), and that its door is genuinely shut — proven by **flooding from the yard**, not by asking the dialogue, because a refusal a walk order can go round is not a door. Then the thing that makes the secret a live one rather than a set-piece: sixty days of invitations take five of Greenrest's own, the town's civil population falls by exactly that many, the rows grow by exactly that many, and the names are kept. And it holds the *shape* of the answer — telling the town costs you 35 reputation, does not touch the Dame, and buys only that the invitations pause while you stand in the square. Finally the pit sells a bout to **one** fighter: three selected and standing at the rail, one sold, driven through `openTalk` (what a right-click actually runs) rather than by calling `openPit` directly. The control names three entrants where this build names one. |
| `demilich.js` | The road to lichdom, which had no road on it. Two of the three ascensions each sit behind five stages and a person — `unclouding` carries `needsQuest:'radiant'`, `sigil_rite` carries `needsQuest:'sigil'` — and **`last_rite` carried nothing at all**: 2,500 gold, a crown, ten remains and a line of italic text under a button, which is what the control prints three ways. There is a third line now and the thing at the end of it is the only creature in the world that has been through the door. The design claim the file exists to hold is that **not one of his five stages can be bought**: it stuffs the stash with fifty of every item in the game and 999,999 gold and shows that `bring`, `hired` and `kept` do not move, while the one stage that reads the stores — the crown — does. Then each ask separately: a risen standing on the exact tile a companion would stand on does not count and neither does the protagonist alone; two hires made *before* he speaks do not count and one after does; the crown is **handed back** (the rite needs it, "a key, not a candle"); a lieutenant eleven days old fails and twelve passes. Then it walks all five through the real buttons — PRESENT THEM → TELL HIM → SHOW HIM → TELL HIM → STAY — and checks the favor is what opens the research node, and that it survives a save. One claim goes through `openTalk` rather than `openImmortal`, because that chain is ordered by specificity and he is undead, neutral *and* furniture — any one of those could have been answered by something ahead of him. It asserts he is furniture: forty AI ticks move him 0.000 tiles and none of eight factions will fight him, `undead` included, because `pierceNeutral` would otherwise hand him to the Purge. It measures the model off the built rig — no arm, leg or spine bones, two lit stones, gold above them — rather than eyeballing a screenshot. And it holds the expensive one: **`spawnDemilich` pockets the world's RNG stream**, proven by calling it twice and checking `seed` comes back where it started. |
| `albedo.js` | The whole of Albedo's quest, driven rather than read — "I cannot ever seem to get it done. Is it genuinely wired up properly?" **It is, and nothing had ever measured that.** One file now runs seizure → gaol → burn → pole → one of yours stands at it → `pyreRead` → her gate opens → `recruitWanderer` → squad 3 becomes 4, through the real day tick and the real `witnessTick`, and every link worked before this change too. What was wrong was reach: every pole in the world stood at the Bastion gate, measured at **777 tiles** from where she was placed, on Paladin ground that hunts necromancers, with the first one going up on day 63. The fix is not a fake pole — the Order simply has a second ground, Saltmere's standing fire, and sends each body to whichever is nearer the town it took them from. Asserts she is reachable and in a town, that her gate is shut on day one, that the standing fire **is still not a pole at worldgen**, and that her refusal now carries a bearing. Its own trap is in the file: the split was first measured by **comparing distances**, which is true on a build where `burnHeretic` sends everybody to the Bastion regardless — geometry is not behaviour, so it takes somebody out of Saltmere and burns them, and the control reads "A SUSPECT FROM SALTMERE WAS STILL BURNED 1059 TILES AWAY". |
| `mother.js` | **She was only in the world if you had ticked the right box on the character creator.** One line in one origin branch — `startTheDescent(me)` under `origin === 'saga'` — both switched her on and chose her cell, so on the other eight starts there was no cell at all: `motherTick` returned on its first line, `listenForMother` returned on its first line, and a necromancer who fought a Deep Warden and broke a barred vault open found a chest. Every claim here is therefore asked of a **Gravekeeper** start, the plainest origin in the game, because that is the run the report is about. It checks she has a cell nobody asked for; that placing her **spent no die** (the cell is derived from a hash of the warren mouth, not drawn — one extra `ri()` at worldgen shifts every roll after it, which this repo has already lost an afternoon to, and `_fingerprint.js` reads identical across the change: 1777 chars, 183 rooms, same towns, same first body); that the ear works for anyone who has one, since `acuityOf` is about being a Hollow or knowing the dust art and never was about an origin; that she stirs once for an ear standing in her warren; that the vault wakes her with no ear at all; that a **stranger** gets a different scene — no CHILD, no price, no road — while one of hers still gets the child scene and the Nascent Rite; and that a reload remembers all of it. 9 red on the previous commit, the first of them being the report itself. The tenth claim — silence for a party with no ear pressing Q by accident — is **green on both builds on purpose**: it guards against noise this change could have introduced rather than testing something it adds, and a guard is meant to stay green. |
| `pit2.js` | Copperhold's working and the ring around it — two things the old world made *impossible* rather than merely absent. Every massif in the game is found by noise and the finder refuses any within `r*1.4 + 20` of a town (a range's cliffs would wall people into their own homes), so no town could have mountains; and `rawDecorAt` sweeps everything wild inside a town's `clearR`, which `seedOre` already has a note about deleting both mining towns' ore. Asserts the thing most likely to break silently first: **`towns[i]` still lines up with `TOWNS[i]`** — Copperhold is placed first with a 130-tile exclusion instead of 95, and town *index* is what `rebelOf`, `isLeader` and a warband's target are stored as in the save, so the order must not have moved. Then that no other pair of seats got closer than 95; that six massifs stand at fifty tiles and none on a neighbour; and that the circle at that radius is **mostly open and not entirely** — "there are passes" alone would pass on a ring that blocks nothing, and mountains you can walk straight through are scenery. Then the hole: 3.9 units below the rim with a worst single step of 1.31 against a body 1.8 tall, no tile of it sealed by the cliff rule, sixteen tiles of copper face surviving the sweep **while the streets around it stay clear**, and a shaft into the undercroft you can reach without leaving a street. The first version of the dig measured **zero units deep** — the town sits near sea level, `groundY` clamps at −0.1, and a four-unit hole in ground already at the floor is not a hole; a town ringed by massifs is upland, so it is put there and the terraces are cut into that. |
| `lantern.js` | Fallowend's violet posts, and the fact that its charms **did not work** — the Doorway Charm has said "Nobody there will tell you whether it works" since it was written, and the answer was no. Measured as behaviour, not as a flag: a real gaunt is given a real quarry and walked at the town, and the metric is **the closest it ever came** — 7.0 tiles under a lit post against 0.8 with the same post out, which is how you know it was the lamp and not the geography. Then the same pair for the Kept Charm (7.1) against an ordinary one (1.0). Then the countdown that is what "struggling" means here in a number: the posts burn `aether_cell`, the town's box is finite, five nights with it empty puts all six out, two cells relight two and then there is no way to make more — and a cell from the player's own stores puts one back up and Fallowend notices. Finally that the Good Kami are not the Church: their stone consecrates nothing (while the building still stands, because the town is laid out around it), so the raise rite answers at Fallowend — the second place on the map where it will. Its own trap is in the file: the first version read where the gaunt *ended up*, which is right for a post (the light is on the lamp, so a turned gaunt is pushed past where it started) and wrong for the charm (the light is on the wearer, so a gaunt closes to the edge and hovers there correctly and forever) — it read 9.0 to 7.5 and called the charm dead about a charm doing its job. |
| `salt.js` | Saltmere's devotion, measured as the thing a necromancer runs into rather than as a paragraph. The town already had the doctrine in its own barks — "cure it in brine and it keeps till judgment" — and was shipping it as a pickling tip. Asserts that a shrine's reach is now the shrine's (`sh.r`) and Saltmere's is 34 against a 23-tile wall, that **nobody else's stone moved**, and that the rite refuses **four tiles outside the gate** — the first version staged that test in the town square and the old build refused there too, because every town has a stone and 19 tiles covers a centre; a claim both builds pass is not measuring the change. Then that it burns its dead at dawn (7 lying, 7 on the fire, 0 left) so unlike every other seat it is not a supply; that the saints are cured at `saltedAt: 0` — **the same `salted` the player can already do to a corpse**, done four hundred years ago — that the dawn fire spares them, that the best of them is 121 in the combat stats against the best ordinary corpse in the world at 24, that **not one answers while the stone stands and they do once it is broken**; and that four Paladins are billeted with posts inside the walls. One claim is about a thing deliberately *absent*: the standing fire is **not** in `pyres`, because that list is read by Albedo's gate and a day-one pole in a walkable town would hand her the name off a fire nobody burned on — undoing the fix whose note sits directly above `pyreRead`. Red fifteen ways on the build before, once every new symbol was reached through `typeof`: the first control died on a bare `saltmere` and reported nothing at all. |
| `orchard.js` | The Aldercott estate outside Greenrest, and what is under its trees. **The secret was already written into the town**: its two civilian barks are "Harvest looks kind this season" and "Never seen a bandit inside these walls. Never will." — one fact said twice, and neither line was written to mean this. Asserts the shape rather than the story: the yard is walled and outside the town's own wall, **at the START town**, with the servant *outside* the gate and the Dame *inside* it (which is the scene — turned away by somebody not allowed to decide, by a person who will not come out to you); that both have their own tree and neither falls through to the watch conversation, which is how a written conversation ships dead and this file already has a note about it happening to the necromancers. Then the part most likely to ship broken invisibly: **the twenty rows are not in the world yet** — coordinates, not corpses, because seeding them at worldgen would put twenty bodies in every save and in the path of every scan in the game for something the player may never find. The door is shut at standing 0 and open at 45, with a dc-20 talk check as the third way. The gift finds them **through the real world tick** rather than by calling the opener. And once opened they are ordinary dead: in `corpses`, `raisableBody` true, raised by the same rite as anything else — no special case, because the whole point of putting it at the start town is that the most decent seat on the map is a mass grave and the player is the one person for whom that is good news. Finally the three endings, each asserted by what it *costs*: exposure takes Greenrest's standing 0 to −30, silence gives it back. |
| `rum.js` | Dustport's house, and the fact that **rum had no verb**. It has been in this game since the beginning as `type:'trade'` — bought in one town, sold in another, with no code path anywhere by which anybody could drink any. So this measures the effect and not the item: that a body at 30 blood is down and a measure stands it back up; that at 34 a sober body is down while a drinking one is still swinging, and at 44 a sober body stays down while a drinking one gets up (both asked of `updateState` itself, not of a copy of the rule); that it **costs** something in `hitChance` both ways — the drinker swings wide *and* is easier to hit — and that it wears off; and that the risen have no throat to put it down. Then the house: THE DROWNED MAN at 11×8 against every other town's identical 8×6, kept by a named barkeep, with Harbourblack **actually behind the counter and nowhere else on the map** — the aether-cell mistake, which this file has now made twice, is that a vendor's list is not a shelf. And the trade: the brew's spread is asserted **against ordinary rum's**, not against a number I picked, because the first pricing made it a 13× money printer (no other town stocks a drop, so scarcity is pinned at its ceiling everywhere and the mults were stacking on top of that). Red seventeen ways on the build before, which took a second pass to arrange: the first version returned early when `DRINKS` was missing and the control reported two lines, saying nothing about the bar, the shelf or the trade — all of which are measurable on any build. | **AND ITS CALIBRATION HAD A RANGE, NOT A VALUE.** `notAPrinter` divides the house brew's spread by ORDINARY RUM's — the right idea — at a bar of 1.35, and went red at 5.5x against 4.1x. Re-seeded rather than bisected: the brew's own spread reads 5.2, 5.5, 5.5, 5.5, 5.6 across five seeds while the staple it is divided by reads 4.1, 4.6, 5.1, 5.4, 6.0. **The numerator barely moves and the denominator moves by half again**, so a bar set just above the tightest calibration observed at writing time was always going to pin red the first time the staple landed at the bottom of its own range. Bar is now 1.6 — the staple's variance plus the speciality's premium rather than the premium alone, still a mile inside the 13× printer the claim exists to catch. A claim is only as stable as the quantity it divides by.
| `taproom.js` | **Three measures, and the third one costs.** The drink limit had a name and the name was `Math.max`: `c.gritT = Math.max(c.gritT || 0, d.grit)` meant a second measure REFRESHED rather than stacked — same wobble, same clock, more blood in the body — so downing shots was strictly optimal and cost only gold. Nothing anywhere in the file read `gritT` out, either, so a player could not tell a sober squadmate from one on its third. The stacking claim is therefore asked at `hitChance`, which is the one place the old bug is actually visible: one measure must cost hit chance and the SECOND must cost more. Around it: the ceiling refuses a fourth by name, there is a real gap between measures, three wear off into a hangover that will not take another drop and swings worse for it, the legs are slower at the ceiling, and `#equip` says the band and the hours out loud. Then the road — Distilling and Cask Ageing as buyable rungs, the Harbour Receipt as a `found` tech with no START button anywhere, the Still's three recipes against their three techs, and both buildings asked of the **BUILD BAR** rather than of `BUILD_TYPES`, because reaching past the door the player uses is the jail.js mistake and this repo has made it. Then the room: its edge tiles blocked and its doorway not, a squadmate at −45 and leaving talked back and kept, one at −85 bought one more night rather than nothing at all, the WARM cap holding from both 20 and 10, and the second sit-down of the same day refused. One trap of its own, and it is worth writing down: the first cut gated every claim on `typeof window[name]` — and a top-level `const` is **not** a property of `window`, it lives in the global lexical environment. Every gate reported the whole feature missing on a build that had all of it. Bare `typeof`, the way rum.js does it. |
| `pit.js` | The gladiator pit at Ironscar — a town whose seat has been called the PIT-BOSS since it was written and which had nowhere in it to fight anybody. Asserts the place before the fight: a 17-tile ring 39 tiles out, outside the wall and the street sweep, off the seam that pays for the town, the pit-master at the gate and four of the house's blades on it. Then **the seal, measured the way the world moves** — a four-connected flood from the middle of the sand with the gate shut, which must not reach outside air (the ring is Chebyshev rather than a radius for exactly this reason: a circle of tiles laid down by distance leaves diagonal gaps, and the warren work spent four measurements learning that a wall touching only at a corner is not a wall). Then a real bout through the real `pitStart`: the stake off the purse up front, the gate down, everybody on the sand, **the fighter hostile to you and to nobody else on the map** including the town that owns the pit, nothing walking in off the waste while it runs and nothing walking out. Then the ending, all of it: the card resolves on its own, the gate goes back up, the purse is paid, the ladder moves, and a body left `down` inside is still lying there afterwards with a walkable gate to fetch it through. **Two claims are staged on purpose and the note is why**: the first run took the three starting characters into THE OPENER, lost, and reported "nothing was paid out" — a true statement about a party that lost and no statement at all about the payout path. Two debtors beating three fresh recruits is the pit *working*. So the win is staged with a crew that can win and the loss with a crew that cannot, because both branches have to be walked and neither should be left to how a fight happens to go. |
| `mere.js` | Hollowmere as a garrison rather than the raise as a spell. The report was "sometimes I feel that they don't properly raise corpses as guards", and the rite turned out to be fine — six paupers up inside a minute of the first morning, the contrary reading having been a live render watched for less than one three-second scan. What was wrong was arithmetic and staffing: **two keepers at a cap of three is a standing dead of exactly SIX**, for the whole game, off a paupers' field seeded once on day one, so a player arriving on day four finds a mortuary with nothing in it. And **the wardens did not exist at all** — "the wardens' fire and the gravekeepers' dead are gate enough" was half fiction in the one town with no wall. Asserts four keepers and four fire-carrying Bog Wardens posted 12+ tiles out; that the six in the yard go up unasked through the real `update()`; that a week of burials takes the town past the old ceiling of six; that the risen are given POSTS and spread rather than kept as a retinue at the mortuary door; that an emptied yard fills again and a yard given thirty nights does not become a pile; and that a keeper's cap and a roamer's are two numbers. One claim stands alone from all of that: **a body staged 7.9 tiles past a keeper's post, with the yard cleared so it is the only thing they can want** — because the leash that drags a posted body back to its flagstone made reaching a corpse a coin toss on `travel` overshoot, measured at 3.5 tiles from a warm start and 5.5 from a cold one on the same build. Red twelve ways on the build before. Its own trap is written into it: every new symbol is reached through `typeof`, because a bare reference made the negative control die of a `ReferenceError` and prove nothing except that the symbol was new. |
| `warrens.js` | The rooms are where the player is now, and the doors are shut. **The lattice you actually walk into contained ZERO rooms**: 81 halls, 94,344 tiles, 81 shafts down, and every chamber, vault, keeper and warden in the game was in seven warrens on floors -2 to -4 behind mouths at the feet of mountains — 35 of 81 halls had neither monster nor chest. The warrens are cut into the halls now, one storey, no mouths, no descent: 28 warrens, 112 chambers, all reachable from a single shaft. **And the doors are real** — on a storey `isBlocked` is `decks.has(k) && !blocked.has(k)`, so a shut door is a decked tile that is also blocked and the movement code needed no changes at all. Getting there took four measurements and each one moved the number: 93 of 106 reachable with every door SHUT (the three-wide bore ate the walls), then 20 with them OPEN (the outward normal came out diagonal), then 28 (exactly one per warren — the wall re-raise was severing the approaches), then 66 (doors landing on CORNERS, whose only interior neighbours are diagonal), then 112 (the approach thread was a rounded line, which steps diagonally). **The same fault at three scales, and it is the rule to remember: everything in this game moves orthogonally, so anything that only touches at a corner is a wall.** Asserts the seal both ways, the barred vaults, and that forcing one takes real time and drives `c.noise`. **And it caught the fifth measurement, which the first four could not express**: 33 of 120 reachable with every door shut on a seed that had shuffled, because the whole loop ran PER WARREN — warren N+1's approach was cut after warren N's ring had gone up and after N's drawn-wall list had been filtered, so it punched through N's ring and left a tile walkable and still painted as rock. The same hole `under.js` sees as a ROCK/DRAWN mismatch. `seedWarrens` now places every warren first, routes each approach to the hall with a four-connected BFS that will not cross a foreign ring or any door, and raises every ring ONCE at the end: 0 of 120 shut, 120 of 120 open, by construction rather than by the tunnels happening to miss. |
| `steeped.js` | What a raised caster keeps. **The rite kept everything about an alchemist except that she was one**: the lieutenant branch copies `stats` off the body and a gift lives in four fields outside it, so a companion with `magic` 20 and destruction at ADEPT came up gift `null`, `att` all zeroes, `arts` gone, `attXp` reset, and `spellsFor` returning an empty list. Magic was also the one stat exempt from `lieuRot` — a flat half — which looked like a rule about casters and was a rule about a number that did nothing. Now the ART carries whole (an adept rises an adept, however far gone) and MAGIC walks down the ladder with atk and the rest, so knowledge and the strength to work it come apart. Asserts the branch survives, the climb to the next tier is not restarted, the first raising takes nothing, she takes the Skeleton Mage rig (`kin='mage'`, gated on a trained branch plus 8 magic LEFT after the rot), and that ordered onto a target she actually casts — driven through the game's AI tick, not by calling `castFirebolt`. Negative control inside the build: a labourer who never read a formula still comes up on the ordinary rig with nothing to cast. Reports, without asserting, that a raised necromancer keeps a binding pool of its own. 7 failures against `c11953c`. |
| `lineage.js` | Where a child is made and where it is born. **A wed player couple on open waste, 250 midnights, conceived ZERO times** — the homestead was a gate at the FRONT, and the child then appeared wherever they stood when the clock ran out. The cost moved to the end: conception is two married people together anywhere at 2% a night (measured 2.5% over 200 nights), a homestead they both stand in is 12%, and she must reach an inn bed or a homestead to deliver. Asserts all of it, plus the season now riding the mother rather than the house, that the wife carries rather than a coin toss, that the term is inside a game-year (6 days against a 7-day year, was 12), that sixty tiles apart is not together, and — the control that matters most — that a townswoman still delivers where she stands, since gating every birth in the world on a bed would have stopped the map growing invisibly. Stubs `snapshot`/`packSaveText`: the day block autosaves the entire world and 318 midnights went from twenty minutes to 8.4s. 7 failures against `e02c308`. | **AND ITS TWO ROOF CLAIMS WERE ONE DRAW EACH.** `aHomesteadIsARoof` came back red three runs out of three while `anInnIsARoof` — same shape, same subsystem — passed every time, which does not look like the 5% `BIRTH_LOSS` and so looked like a regression. It was not: `rnd()` is the **seeded worldgen stream**, so a 0.05 roll at a fixed point in it is not a one-in-twenty chance but a **constant for the build** — once something upstream shifts the stream far enough, that draw is under the line forever, re-running returns the same answer, and bisecting points at innocent code. Proved with `?seed=`: default seed loses on night one every run, seeds 7, 21 and 99 deliver and the house takes the tally, and twenty re-staged terms on the default seed give 20 born and 0 lost. Both roof claims now put her back at her term when a night takes the child and ask up to six times, and say so in the pass line — the inn claim too, although it was green, because it had the identical exposure and was green only by where this build's stream happened to fall.
| `cradle.js` | What a child is born with. **Every child in the game was the same child** — atk 1, def 1, tough 2, no gift, and no stats handed out at coming of age either, so two hundred births made two hundred interchangeable people. Now a birth is an event, and every rate here is asked against its own control in the same build: a loss rate (3.0% delivered on time) against the same rate a fortnight overdue (22.5%, which is the half the player controls); an inheritance rate (35% from one gifted parent, 57% from two) against two ungifted parents (zero); a dual gift at 8% of births, wide not deep since `attCap` reserves master for the branch you ARE; a moonborn rate of 50% under a blood moon against 3.6% without. Also asserts a gifted newborn can cast NOTHING and comes into the branch at sixteen through the real coming-of-age block. Two harness lessons in here: `makeChar` already spreads newborn stats (a `tough > 2` threshold read 55 of 194 against a 4% rule — read the `strongborn` flag, not the number), and no fertile line in this build is deaf, so the `canHoldGift` guard is staged by hand and says so. 12 failures against `89fbd0e`. |
| `lean.js` | A save only writes what is not the default. **`chars` was 97.9% of a 4213KB save** — 1046 bodies at 4036 bytes each, of which 1138KB was the word `false`, 938KB was `null`, and 618KB was a `parts` block with 7322 of 7322 entries untouched. `restore` reads every one of those through a default (`!!s.foo`, `s.foo || null`, `s.foo ?? null`), under which a missing key and a saved `false`/`null` are the same answer, so they are no longer written: 4213KB → 2017KB, and old saves load unchanged. Asserts no default is written, the size fits a phone, the one exemption (`construct`, whose restore falls back to the RACE default) survives as `false` on a golem, and that everything true survives. Its round-trip claim asks about the SET OF KEYS rather than values — a first version compared every field and went red on boss stats being re-derived, which is drift, not a dropped key. That narrowed claim is what caught `w_sw` and `w_dag`: two weapons seven spawn tables hand out that were never in `ITEMS` at all. 2 failures against `380bee1`. |
| `slots.js` | Six places to put something, and three ways to put it there. **The starting point was in the wrong place**: `helmOf` resolves to `helmKind(c)`, which reads a body's FACTION, so the Order's great helm was a costume the renderer put on — uncomeoffable, unlootable, and still worn by a Paladin who lost it. Asserts six slots off one table (`slotFor`), that the quick-equip survives AND now files a helm on the head rather than as body armour, that five formulas move with the same body's gear on and off (`chaOf`, `rawMaxMana`, `invCap`, `risenCap`, `armorPen`), that hat+cloak+trinket cap under plate, that `openInventory` draws a figure and all six slots and filters the wagon to the one you pick, that a Paladin's crest is an item that comes off the body, that the Armoury kits twelve bodies in one click without downgrading the one already holding a nodachi and does nothing on a second click, and that the lance-and-gifted-hand rule is obeyed from all three doors. Everything driven through the panels a player uses. 16 failures against `36ddf34`. |
| `kitted.js` | The new slots have to be REACHABLE, not merely defined. **The aether-cell bug, repeated**: head, cloak and trinket were on vendors' lists while no town anywhere held one — 3 of 3 types listed, 0 of 7 towns stocked, and 0 recipes in the game that made one. Asks the four questions a player would: can I buy it (listed AND seeded AND drawn on the counter — three separate things that can be wrong), can I make it (a recipe per slot, and a smith at a real forge turning out four helms through `craftTick`), can I take it off a body (95 bodies wearing one, and `lootCorpse` yielding it), can I find it in a ruin (400 chest rolls). Plus grades and weathering. Its render claim is the one to read before writing another: `buildCharMesh` returns the entry not the group, `isMesh` misses merged boxes, vertex counts collide, and a position hash across different bodies is confounded by faces drawn off `c.id` — one body, mutated, hashed, with a pack as the metric's own control. That version caught the Road Cloak and the Gravecloth Cloak being identical geometry in two tints. 7 failures against `d435257`. |
...
| `standoff.js` | **Two ways to be in a fight and not be fighting**, and a lesson about sample size. A body walks to where the code told it to stand and that place is outside where it is allowed to swing or shoot — so the file never asks "did it win", it asks whether the place a body settled is inside the range it needs. Sweeps five monster sizes, four ranged weapons and four gifts. **One thirty-second fight is one sample, and this file learned that the hard way:** the size sweep staged one fight apiece and compared two single numbers — is what size 1.3 took at least half of what size 1 took — and went red in a full suite reading 60 against 127, which looks exactly like a reach dip at one size. Eight repeats a size says otherwise: size 1 lands 89-159 (median 123), size 1.3 lands 67-155 (median 125). No dip. The DENOMINATOR alone swings 1.8x, so the ratio can fall under a half on a healthy build whenever the numerator draws low and the denominator draws high — 67/159 is 0.42 and both are ordinary. Medians of five now, bar 0.6 of the ordinary-sized control at every size (observed 1.00, 1.04, 1.02, 0.97, 0.78 — the last a wyrm, honestly harder to stay inside reach of), and it still catches the fault it was written for, which measured **eight** blood off a wyrm against 104 off a man. Costs 137s against 23s, which is 2% of the suite and worth it. |
| `_reachdip.js`, `_barspread.js` | **What does this measurement actually do when you take it more than once?** Two scratch benches written to answer three reds in one suite, all three of which were bars set flush against a single sample. `_reachdip.js` repeats `standoff.js`'s size sweep eight times a size and prints every draw with its min/median/max. `_barspread.js` does the same for `sundered.js`'s regrowth clock (every site, four seeds) and `wards.js`'s fleeing gaunt (twelve runs, sampled every second for five). Reach for these before bisecting any red whose failure signature is a number that came out of a rolled or seeded process. Not in `run.js`. `node tools/_reachdip.js [game.html] [repeats]` · `node tools/_barspread.js [game.html] [repeats]` |
...
| `grafts.js` | What a graft is worth, asked of every place that reads the table. `PROS_TIER` and `prosVal` were declared BETWEEN TWO STATEMENTS OF `moveSpeedRaw`, which made them locals of that one function: legal, silent, and invisible from the two other readers. A grafted arm threw `prosVal is not defined` on every swing — from inside `update()`, which unwound the rest of that sim step with it — and the WEAR button consumed the graft, marked the limb, and threw on `PROS_TIER[tier]` before the log line, so the player saw nothing happen. Twelve days and no probe; `moveSpeedRaw` worked the whole time because the table was beside it. Asks each reader on its own and drives the one that matters through the real loop: a grafted fighter beside an enemy, with the signal being a swing REACHING `applyDamage` rather than the foe's hit points — the first version counted blood settling toward its maximum as three blows and went green on the broken build. Found by ESLint `no-undef`, which is the argument for running one. |
| `maws.js` | How often the Maw asks, and whether it still hears the answer. Chrome's sampling profiler put `mawQuarry` at 45–51% of EVERY SIM STEP on day one with six corpses in the world, and two things composed into it: the search walked the whole roster again for every candidate (`chars.some(m => m.mawTarget === o)` inside both loops, so one search cost (down + corpses) × chars and grew with the field), and a searcher that found nothing asked again next tick — 150 of 151 `eatsDead` bodies, thirty times a second, for the same nothing. A claimed-set built once and a 1.5s retry after a miss. Measured interleaved three times each: `simcost.js` 17.0 → 8.0 ms a step direct, 36 → 19 in the soak; searches per 90 steps 5,435 → 269; and one search with a hundred staged corpses 2,030µs on the old build against 90 on this one, because that is the half the retry cannot fix. **The count is the assertion, not the time** — it holds under suite load — and then the behaviour is asked directly: a corpse put down in front of a Maw that has already missed is taken 1.6s later, and two Maws still never claim one body. A timer that never expires would also have made the sim faster. |
| `wards.js` | Who is asked about Ainzopha'ar's Light, and whether the light still works. Three copies of one loop — `wardedFrom`, `wardSears`, and an inline IIFE in `physics` asked for every gaunt every tick — each walked ALL of `chars` per call to find the two or three bodies (on day one, none) holding a ward: 176,000 `isConcentrating` calls a step, 165 a body. **The audit named only the first**, because a sampling profiler files a loop's cost under its callee and the inline copy had no name at all; the first fix covered `wardedFrom` alone and measured NOTHING (8.19 → 8.31 ms a step, inside the noise), which is how the other two were found. One candidate list gathered in `rebuildCharGrid`, one reader (`lightNear`), `startConcentration` adding a light raised mid-step. Measured interleaved three times a side: `simcost.js` 8.5 → 6.8 ms a step direct, the sampling profile 8.4 → 7.1, and the predicate count 176k → 0 a step. **Counts are the assertions** — bodies asked per call, and per step in the live world — because a timing under suite load is a coin; then every way the list could go stale is asked directly, since a list never rebuilt would ALSO be fast: lit this tick with no rebuild, let go this tick with no rebuild, the caster down, the far side of the line, the wrong storey, out of range, a gaunt still turning from it through the real loop, and a save round trip. **AND ONE BAR WAS SITTING ON THE FLOOR OF ITS OWN SPREAD.** `andALesserGauntStillTurnsFromIt` wanted the gaunt more than a tile further off after ONE second and read 0.8 — with the other half of the claim, letting go of the quarry, passing. Measured across twelve runs on four seeds, the gain at one second is 0.8 to 3.8 tiles: a body that has to stop, turn and set off spends most of the first second not going anywhere. Two seconds now, floor 1.19 observed, bar 0.5. And the distance was always the junior half — one run tops out at 1.2 tiles and stays there for four more seconds, which is a gaunt that turned away and stopped at the edge of the light, i.e. exactly right. |
| `quiet.js` | Whose work the feed is for. Nine necromancers in a fresh world are not you — two Guild, four town, three drifters, six of them posted keepers — and `castRaise` never once asked who was casting, so every refusal and every success any of them made went into the player's log. Four claims, and the first is the one that makes the rest mean anything: prove somebody else really is raising the dead before asserting you are not told about it. Then twelve staged raisings by keepers — **six genuinely refused for a full binding and six that stand a body up**, counted separately so a staging that only exercised one path fails as a staging fault rather than passing as a fix — take nothing out of the feed, while your own caster at a full binding still says so **by name** (`binding is full`, not merely "some line arrived": a raising also trips `crime`, and "GREENREST wants you for raising the dead" would pass a bare length check while the refusal stayed silenced). The fourth measures the WORK rather than the words, because silencing a log is exactly how you hide a loop: the keeper tick gated on `risenCount >= 7` while the rite gates on `risenLoad + 1 > risenCap` — about five for a town keeper — so a keeper at the real ceiling passed the first gate, walked to a corpse, failed the rite, dropped the claim and came back three seconds later, forever. It stands three heavy bodies up so the LOAD is 13 against a ceiling of 5 with the COUNT at four of seven, wraps `castRaise` to count the rite being ATTEMPTED, and runs the world's own tick: **3 attempts on the build before, 0 on this one**. Three fixture faults worth keeping, all of which produced a green on the broken build. Both binding-full states were staged with `k._bindW`, which `bindStrain` reads and `risenLoad` has never heard of — one claim printed `wouldRite: false` inside its own failure text while asserting a keeper past its ceiling, the other watched the rite succeed and read the crime line as proof the feed still spoke. The keeper claim sampled `necroBody` once after ten seconds, which is a coin toss: the claim is cleared on the tick the rite runs and retaken three seconds later, so the reading lands in the gap about as often as on the claim. And the scan measures the corpse against the keeper's GROUND rather than the keeper, so a body at the feet of a keeper posted out on its leash is never a candidate — and once it was staged on the town centre instead, one of the five other keepers with room simply raised it four seconds in, and the count came back at one. They are stood down for the length of the claim. The silence claim also counted ALL feed traffic at first, catching two regard tells ("Dax is having second thoughts") fired because a hand of yours stood watching somebody else's necromancy — which are the player's business and must keep arriving, so the claim matches the raise path's own vocabulary and reports the rest in the pass line rather than discarding it. |
| `queue.js` | **The benches take one job at a time, and the list behind it pays as it goes.** A chain four deep, each rung gated on the one before, run through `researchTick` with the rate stubbed — `researchRate()` is zero until somebody is sat at a built bench, and the first version of this set `research.left = 0` and called the tick, which returned on its first line, so the chain "advanced" through six identical entries while the queue was broken. **MONEY IS NOT THE ONLY BILL** is the lesson this file keeps relearning: it stocks the shed with every material a tech can be billed for, because a probe that stocks gold alone watches the queue hold — correctly — and calls it a failure. That trap sprang a second time on the same line the day Deeper Rites started wanting a piece of Sundered Marrow. Claims: a chain can be lined up and works its way down; nothing is paid for up front and each is paid when it begins; a stalled queue says why; a head it cannot start does not block the thing behind it; the button is on the panel and comes off again; and the list rides a save in order. |
| `depths.js` | Three storeys under the world, and the warren bands that had never once run. Fourteen claims, every one **NOTHING TO MEASURE** on the build before. The shape first, because three copies of one map would be three times the walking and none of the descent: 81 halls on 139k tiles, then 36 on 90k, then 16 on 56k, at -22/-36/-50 — each a thinner net than the one above, and each a real drop below it rather than the 3.4-unit crawlspace `UNDER_DROP` exists to prevent. Then the claim that makes the rest worth anything: flood each storey from a stair that ARRIVES on it and require the next stair down to be standing on ground the flood reached — 81/81, 36/36, 16/16 halls, with 111 ways into the ground, 16 on to the Deepworks and 7 to the Sump, because a way down you have to find is most of what makes the bottom feel like a bottom. The first cut of that placement shipped **two** ways to the Deepworks and none at all to the Sump, which this claim is the reason anybody knows. **Section 5 is what the file is for**, and it counts BODIES IN `chars` rather than reading a table, because the last of three stacked faults was a constructor that built a perfectly good bandit and never added it to the world — a claim that read the branch, or the table, or even the returned object would have passed on a build with a hundred and one empty rooms in it. Shallow: 276 bandits and no Larder-Kin. Deep: 32 Larder-Kin and no bandits. Both ends asserted, because the positive alone passes on a build that puts bandits everywhere. Then 120 of the 276 holding a torch instead of a weapon; a hall chest worth 510 → 1562 → 2134 going down; the Tooth held by the one Unblinded on the deepest floor there is; a squad ordered to the bottom arriving through 0 → -1 → -2 → -3 on one order; bodies and warren rock surviving a save on the storeys they were on. Two claims guard the seams the change could have silently torn: every wall, door, room and body of all 45 warrens on its own cave's floor (a warren cut at -1 around a hall from the Sump's lattice is walkable, drawn as stone, connected to nothing — the leak this repo has fought from two sides already, reintroduced from a third); and the depth-folded bucket key answering with its own storey's ground at a hall on each. That last one is asserted on the INDEX rather than on a mesh count, because `flushBoxBatch` merges the window into a couple of geometries and a mesh count reads the same whether the floor plate is fourteen thousand tiles or one — it cannot tell the failure from the success. **AND IT HAD BEEN PASSING BY LUCK FOR ITS WHOLE LIFE.** `andASquadOrderedDownArrives` starts a body on a surface stair and orders it to the bottom. Its own comment said what was under test — *"the CHAIN of three descents, not the pathfinder"* — and then took `stairs.find(...)`, the FIRST surface descent in array order, which drops you into whatever region of -1 that stair happens to open on; only some regions of -1 have a shaft onward within reach. So it was the pathfinder, and the geography of one world at that: **the control fails seed 7 and passes 91 and 404.** Measured from the surface stair nearest a bottom shaft, it landed on -1, walked 433 tiles in six hundred seconds of sim, and the nearest onward shaft was still 453 tiles away — nothing about which is a fault in a descent. The three links are driven one at a time now, each from its own shaft: deterministic, and red only if a descent is genuinely broken. How far apart the shafts sit is printed as colour, where a per-world number belongs. `theToothIsAtTheBottom` was the same shape and turned out to be a real game bug — the seeder gave the Tooth to the deepest Unblinded that happened to exist, and one is seeded into 14% of inhabited halls, so across seeds it read -3, -3, -2, -1. A one-in-four chance the bottom of the world held nothing worth the walk. The holder is moved to the deepest floor now. Takes a seed argument: `node tools/depths.js game.html 91`. | |
| `cold.js` | What a storey nobody is on is allowed to cost. The premise is counted before anything is asserted, because if it stops being true the tier should go: of 1,748 bodies, **1,133 on a storey with no player unit on it, and only 166 of them idle** — twelve hundred bodies walking and fighting four storeys under somebody who cannot see any of it, which the old `_far` tier could not touch because it only drops a body with no quarry. So the gate is the STOREY, and two claims pin the edges as exact zeroes: nothing of yours is ever cold, and nothing on a floor one of yours is standing on is either. One body walked down to -2 wakes all 326 of them and leaves -3 asleep — per-floor, not "underground is awake". Then `stepToward`, which tested collision at the destination and nowhere between: exact while every step was a sixth of a tile, and a hole in the world the moment one is a tile and a half. It is driven at 1/30s, 0.3s, 1s and **3s** — far longer than the tier will ever hand it — because a bound that only holds at the rate it was tuned for is not a bound. Then the two things the slow clock must not cost: ground covered (the same bodies, cold against warm, 0.93x) and a fight (twelve bodies and a total, A/B at the same spot). Its header carries the **memo that was built, proved exact on fifteen thousand pairs, and thrown away for being four times slower** — a cache cannot reduce a call count, only what each call does, and `hostile` dies on one string compare. Four of its own faults are written into it: `_lod` and `_cold` were conflated, which broke every fight underground (`separate` is where `_contacts` is written, and it skips pairs where both are `_lod`); the arrival claim shoved a `moveTarget` into a gaunt, a field the body's own ai owns and cleared within the second, so it measured the ai and reported 2.2 tiles of 486 while `physics` ran at exactly the right rate; the fight claim staged one pair and asked whether either died, which is far too noisy — the same pair on a warm floor came back at 72, 85 and 86 blood on three runs and never killed anybody either; and the wall claim searched one room's north ring, which is where the approach tunnel usually arrives. **And a fifth, which stood red for two sessions:** the melee that replaced that pair staged six bandits against six cannibals, and `hostile('bandit','cannibal')` IS FALSE — outlaws do not quarrel with outlaws. Twelve bodies stood in a hall for forty-five seconds and the only blood came from a shrike that wandered in: 2 cold, 0 warm, identically on every build going back to before the cold tier existed, while the header note blamed the warm control's variance. Bandit against gaunt is a real enmity and also the fight the depths actually hold, and it comes back 795 cold against 645 warm with twelve of twelve down — so the ratio the claim had given up on is usable again, and says a fight nobody is watching resolves at the same rate as one that is. **A staged fight must assert that its two sides are enemies:** `foe` is checked first and separately now, because a melee between two sides that will not fight reads exactly like a working build with a quiet floor. | **AND THE TIER MOVED, SO EVERY CLAIM HERE IS RESTAGED.** These were all staged on a warren storey with the squad up in the light — ground that no longer exists, because a storey below ground with nobody on it is now frozen (`frozen.js`). Staged that way the file went comprehensively red: 0 blood either side of a staged fight, 0 tiles walked, and a census finding **eleven** cold bodies in the world, all on tower decks. The claims were right and their ground was gone. The tier is the mirror image now and matters more: take the squad down a hole and the whole **surface** is a storey nobody is on — which may not freeze, because that is where the economy is, and may not run at full price either. All nine claims now stage the bodies under test on floor 0 with the squad underground, and read better than they ever did on the Sump because a surface staged that way has six hundred bodies in it: 607 of 607 go cold, they cover 0.82× the ground of a watched floor, and a staged fight of twelve takes 692 blood against 735 watched. Restaging a red claim onto the ground its law moved to is not the same as weakening it to make it pass — the alternative was deleting the claims, and a tier with no claims on it breaks silently.
| `frozen.js` | **A storey nobody is on does not tick at all — and it was 71% of the sim.** Measured before it was built, because `_cold` already throttled the depths and the question was whether throttling had left anything worth taking: 1145 of 1777 bodies below ground, 41% of physics calls and 30% of ai calls going down there, a step at 8.66 ms against 2.50 ms with the depths lifted clean out. The claims: nothing below ground thinks or moves while nobody of yours is down there (counted at `ai` and `physics`, which is where the claim lives); they are not in the grid either; **the surface is untouched**, which is the half that makes this safe rather than cheap, because the economy is up there; your own garrison keeps its clock on a storey you left; walking down thaws it in one step **with no accumulated debt** dumped into the first tick; and a frozen warren does not quietly die of old age — 482 two-hundred-year-old bodies survive three nights, which is [109] arrived at from a different direction and the one way this change could empty the depths without anybody seeing it. Result, each step measured: 8.66 → 4.96 (skip the work) → 4.47 (out of the grid) → 4.35 ms (iterate a live list). Half the sim. |
| `grid.js` | **The spatial hash was rebuilt seventeen hundred times a step to move fifty-five bodies.** `rebuildCharGrid` cleared a Map of ~1777 entries and refilled it on every sim step — one of three whole-world passes that run once per *step* and so multiply by the speed button, which is the mechanism behind "the higher speeds are where the FPS tanks". `CELL` is eight tiles and a body walks ~3.4 tiles/s, so it crosses a cell line about once every two and a half seconds: **counted, 55 of 1777 bodies change bucket in an average step.** Bodies are filed now and stay put until their cell actually changes. The claims are **counts, not milliseconds** — a shared container cannot separate an 8% change from its own noise (eight interleaved pairs were needed to say 10.325 → 9.297 ms median, winning 6 of 8 rounds), but Map operations and bucket membership are matters of fact. It asserts the movers are a small fraction, that the filed grid holds exactly what a rebuild from nothing would, that a downed body is unfiled, that a body spliced out of `chars` by any of twenty-odd call sites is **caught** rather than left to haunt `charsNear`, that `restore` regrids, and that the dead `_phSkip` flag is gone. Two traps of its own, both recorded: censusing straight after `update()` returns reads mid-step drift (the pass is the *first* thing in `update`, so the invariant holds at the top of a step, not the bottom) — and the first control ran **no steps at all**, which made `stillExact` red for that reason and `noDeadFlag` *green* because nothing had run to write the flag it was hunting. A control that doesn't exercise the build is not a control; both builds get the same forty steps now, and the old one reports 1766 bodies carrying the dead flag. |
| `rim.js` | **The world stops being a square, and it stops gradually.** "I hate that the map suddenly and sharply ends, and in such an obvious square shape." The obvious half is the shape — the old coast ran off `Math.min(x, y, W-1-x, H-1-y)`, which is a square by construction, so no amount of water would have given it corners. The half nobody had noticed is that **there was no sea at all**: measured on the old build, the outer forty tiles are **15-18% water**, scattered puddles, and you can walk to x=0 on dry ground. Out at the rim the old formula stopped ADDING dryness rather than subtracting it, so a tile still had to draw its own noise under 0.34 to be wet — 17% of the time, everywhere, forever. The comment sitting above it read *"the far water is why nobody leaves"* and it never made one tile of solid water. Now a superellipse radius with its own shoreline noise, and a band structure: brine, then salt crust, then the dead salt, the outer two lifted clear of the water plane so they read as ground rather than as more ocean. Claims: the ring is solid (100% at 10 and 30 tiles in), the corners drown **4.2x deeper** than the edge midpoints so the landmass has corners rather than a frame, all four rays inward read `3>2>1>0`, the pans stand above the plane while 100% of the brine is under it, and every band still blocks — because the bands are a *rendering* fact carried in a parallel `brim` array while the blocking is the `terr === 3` that thirty-odd existing checks already read. The last claim is the one that matters most and is green on both builds: five seeds, every town on land, one landmass each. **Two faults of its own, both from sizing the bands wrong.** First cut measured them in superellipse units — `rad` runs to 1.0 at an edge midpoint and 1.19 at a corner, so the bands were four times wider at the corners and the dead salt **did not exist at all** along the middle of any edge. Distance-to-shore in real tiles, by two-pass chamfer, is the only measure that gives three bands of one width. Second cut used 22 and 55 tiles, which does not fit: the land reaches within 43 tiles of the edge on one side and 66 on another, so all three bands have to fit in the **narrowest** rim the coast ever leaves. 14 and 32. And the pan-height claim first sampled band tiles including their seams — `HV` is per-vertex and averages four tiles, so a crust vertex next to brine mixes -0.15 with -0.55 and lands under the plane, which is a beach doing exactly what a beach should. It samples band interiors now. |
| `waylines.js` | **The bones of a dead god, and a road nobody walks.** Two notes that meet at one material. Sundered Marrow had three uses and every one of them was somebody else's — a price at a counter, regard with the Vigil, two pieces for a scholar's favour — so it is a research material now on the deep end of the necromantic road, and it is also what a wayline is cut with, which is why both halves are one file. The teleport design is the user's call verbatim: **expensive to build, free to use, and somebody trained has to work it** — and not a necromancer, because "anyone with alchemical training should be able to work it", so the test is `hasArts` and a divine or dust squad gets waylines on the same terms as a dark one. Thirteen claims: four rites want marrow and the bench actually **spends** it (2 out of the stores, measured) and refuses to start without it; the circle is offered in the **BUILD BAR** and locked behind the tech, which is the cell/homestead mistake this file has now recorded three times; an empty stone refuses by name; a divine caster works it and a mindless risen does not; tier I moves one body and THE HOST ROAD moves the selection; a hauled corpse crosses with its carrier; a body that has just been hit is refused, so a losing fight is not a button; it crosses storeys; it rides a save; and the two circles **share no colour at all**, which is the note's own "we'll need to find a way to make it distinct" asked of the authored geometry. One staging fault worth the line: the marrow claim first picked the heaviest cost, which is The Last Rite — and that rite is gated on the demilich's favour, so `researchBegin` refused on the quest and the claim reported "the marrow is a label" about a build where it is nothing of the kind. **A claim about materials has to be staged on a tech whose only obstacle is materials.** 13 red on the control. |
| `order.js` | **The road is open to anyone not walking with the dead.** "It doesn't make sense that paladins are immediately hostile to you if you just pass them on the road as a normal human... Otherwise I will never be able to enter Saltmere or complete Albedo's quest." And they were, for a reason worth writing down: hostility toward the player ran off `menaceFlag`, a **single global boolean recomputed nightly**, and one of the three things that set it was `c.stats.magic >= 15` on ANY party member. So the flag was true from the first night of a caster's game, it was the same value for every body in the squad, and a hired swordhand was hunted for somebody else's gift. Ten claims against the layers: a plain hand is walked past, a MAG 40 caster of any gift is still just somebody on the road, the same hand standing beside a risen is read as a party with the dead in it, a **Shrouded** risen is not (the art that hides the dead from a town now hides them from the Order), the risen itself is hunted exactly as before, notoriety is the gate and mere fame is not, spilled Order blood is known regardless of gift, and the flag is about your HOST — nine bound risen, not a quiet MAG 60. Plus the siting note: five worlds, the Bastion 43-61 tiles from Saltmere and the nearest seat every time, walls and gaol and garrison intact. **One staging fault, and it is the general kind:** the two walked-past claims came up GREEN on the control, because `menaceFlag` is only recomputed inside `update` and the harness pauses on arrival — so the flag was `false` on both builds and the claims were reading an uninitialised world, not a rule. Fixed by parking a MAG 40 dark caster six hundred tiles away and driving forty real steps, which lets **each build set its own flag its own way**; the control now reads `menaceFlag = true` and both claims go red there. 6 red on the control. |
| `flare.js` | **Fire that looks like fire, and a wyrm that fights.** Two aesthetic notes, one file, because both are claims about what a frame contains. `addSpecks` was the entire visual language of the game — four to twenty 3-pixel SQUARES of one flat colour on random vectors, fading over half a second — so a firebolt landing and a bandage being tied were the same event with a different hex code. Eight claims. Fire: it **rises** and keeps rising while an ordinary speck is falling; it **cools** along a ramp from white through orange to smoke, because a flame is a journey between three colours and not one of them; it **swells** 3.2x from root to tip; it **burns every frame** (8 live tongues on a burning body, emitted by the draw, so it burns continuously instead of puffing once per damage tick); and it **spends no dice** — sixty motes move the seeded stream not one step, the `rnd`/`vrnd` rule that `updateDust` already cost this file an afternoon over. The beast: it rears on the wind (head 0.38 up, front legs 0.67 off the ground), the strike throws it 0.39 forward, and it puts itself back afterwards. **Three staging faults worth the lines.** The rig table is `charMeshes` keyed by `c.id`, and rigs are built only when the renderer LOOKS at a body, so a wyrm staged four hundred tiles away has no rig to read — it goes beside the party. The windup is consumed by the SIM and the pose is written by the DRAW, so stepping to get a frame destroyed the thing being measured; the page renders while paused, and only `update` stops. And the idle guard first measured against `e._headY0`, a field only the new code writes — on the control it compared the rest pose to itself and reported green about a build with no animation at all, so it now says **"cannot speak"** there instead. **And a fourth, which is the house rule verbatim:** the rising claim's contrast half asked that "more than half of ten" specks had fallen BELOW their start, and `addSpecks` opens with `vh: rnd()*2.5` — gravity 6 over a 14-update window takes 2.8 off the velocity, putting break-even at v0 = 1.4, which is 56% of a uniform [0, 2.5). That made the bar a **47% coin flip** and it duly came up tails. What is not a roll is that 2.8 exceeds every launch speed the function can produce, so every speck ends the window travelling downward while a tongue's `vh` only decays toward zero and can never change sign: the claim measures the **sign of the velocity** now, not a count. 7 red on the control, and the eighth is that guard declining to speak. |
| `sweep.js` | **The two orders you give a band in the field, and what they actually do.** Both reports come down to the order measuring the wrong quantity. The PATROL was handed `r = 26` and its legs rode 0.55-0.91 of that across five waypoints — a ring 14 to 24 tiles out, inside the sight range of the base it was screening, which is the whole of "it's literally just walking around in a circle". The FORAGE asked two questions, both about objects that already exist, and ended the instant both answers were no — so on ground you had already walked it fizzled on the FIRST TICK, because it was never looking at the ground at all. Five claims. The patrol is enumerated straight out of `cmdLeg`, which is a pure function of the order, so a circuit is measured without a step of sim: it now stands 52-58 tiles out, runs 335 tiles round, and is walked in eight legs covering 97% of its ring rather than five covering a pentagon. The forage is driven through the tick on the exact staging the report describes — every chest and body inside the order marked taken — and the band walks 45 tiles out and maps 241 unwalked lattice points on the way. **Three staging faults, and the second is the ledger's rule again.** The radius lived in the CHARACTER PANEL (`k === 'hold' ? 10 : 26`), so a harness calling `giveCommand` got 26 whatever the menu did — it is on the order in `CMD` now, which is where the size of a duty belongs. The first cut checked `vis` at the destination AFTER the run, by which time the band had walked there and looked at it. And counting ground learned is not enough on its own: a body sees `17 + dayness*13`, up to **thirty tiles**, so on a 26-tile forage circle a band that never moves has already mapped half of it and the claim went green on the control about an order that ends on tick one. The bar is now how far they GET — past 34, beyond what anyone can see from the middle in the brightest light. 4 red on the control, which says "Nothing here worth carrying" and travels 1 tile. |
| `pitch.js` | **The warrens have fires in them, and the fires stay lit.** "Bandits in the underground layer do not use torches at all and thus suffer a horrible penalty." The interesting part is that worldgen already guaranteed one torch per chamber, with a note above it explaining exactly why — *"a room of blind men is not a fight, it is a chore"* — and the guarantee was defeated by a clock somewhere else entirely: a torch holds `TORCH_HOURS` and `torchTick` only ever relit for `faction === 'player'`. Every world was correct for its first ten game-hours and blind underground forever after. **Which is why the central claim is about TIME** — asking "does a chamber have a light" on a fresh world passes on the old build too, so the clock is driven with a full day and the question is whether anything is still lit. Five claims: every shallow chamber carries a standing fire, `litNear` answers with it for a body holding nothing, a day of burning takes 142 lit torches to 142 rather than to zero, and of the 239 people standing in their own chamber not one is in the dark. **Two things the measuring changed.** The fire is sized to its ROOM rather than to `TORCH_POST_R`: chambers run to 18x14, whose half-diagonal is 11.4, and a centred fire at 8.5 left 39 of 315 dwellers outside their own light. And the claim was re-aimed — the first cut asked that NOT ONE warren-dweller anywhere was in the dark, which nothing ever promised: only one in three carries a torch, and a man forty tiles up an unlit corridor is in the dark because that is what a corridor is. 4 red on the control: 142 torches to 0, and 239 of 239 blind. |
| `roster.js` | **Who the interface is actually pointing at.** Three reports, one question. The cursor reached through solid rock — and the floor guard in `bodyHit` was present all along, written `activeFloor < 0 && ...`, so it only ran WHILE YOU WERE UNDERGROUND; standing on the surface, the ordinary case, it was skipped and all thirty-six hit tests in the file fell through to a plain 2D distance, where a body four hundred metres straight down is exactly as close to the cursor as one at your feet. You could talk to it, target it, heal it and select it. Six claims. The floor is asked of `bodyHit` itself, both directions, with a **guard that the fix has not eaten the ordinary case** — green on both builds, which is the point of it. The squad claim goes through the BUTTON and not through `assignToGroup`: the folders already worked, what was missing was a road from a world selection to them, so the road is what is tested — click SQUAD, take the menu it builds, and all three selected land in one named squad. And the goal bonus is asked of `xpGain`. **One staging fault worth the line:** that claim first built two probe bodies and compared them, and `xpGain` opens with `amt *= raceOf(c).xp` and multiplies again by `subOf(c).learn[stat]` — `makeChar` ROLLS a race, so the two differed by 1.3x before the goal was consulted and the claim reported the goal paying NEGATIVE seventeen per cent. One body, run twice, differing only in the field under test. 5 red on the control, where the goal pays 17%. |
| `ledger.js` | **What a town writes down about you, and what it does to the people it is holding.** The gaol one is two correct systems meeting: `jailedAt` pins a body at the cell and returns out of the step function before anything else can happen to it — no orders, no moving, no swinging, no fleeing — while the town clause in `hostile` makes any player body a target once `rep < -50`. Your sentence became an execution, carried out on somebody who could not raise a hand, by the people holding them. The bounty one is a number that only went up (and which everything reading it saturates against — the watch's odds cap at `bounty/500`, a search at `/700` — so past about nine hundred it changes nothing and climbs anyway, while settling costs 1.5x to 2.5x of it, making an uncapped bounty an unpayable one) plus a line of log per offence. Five claims: a prisoner is not hunted either way round, **the same body standing free in the same street still is** (the guard, green on both builds — the fix must not have made you safe everywhere), thirty murders stop at the 2400 ceiling instead of reaching 12000, the same three offences that print three lines in peacetime print none once the fighting starts, and the price goes on being added up through it. **One staging fault, and it silenced two claims in the direction of success:** `crime` suppresses an echo and a bounty that has not moved, and the ceiling claim had just left the town pinned at the cap with `_cryKind` stamped at that same hour — so nothing was printing before the assault either, and "nothing printed" was being read as evidence. The premise is counted first now: the peacetime line count is the claim's own control, and a zero there says **"cannot speak"**. 3 red on the control. |
| `water.js` | **What the Golden Age threw away, and what it was after.** The inversion is the design: the Golden Age desalinated and discarded the salt, the survivors evaporate and discard the water, and Saltmere lives on the waste product of a dead civilisation. Nine claims across the chain — the shore gives fish and brine and the pans give nothing; one job decides which of the two a hand comes back with; a well is refused on the salt and draws 5 a day inland; the pan boils 3 brine to 1 salt with no tech; the plant is locked behind a **found schematic** with no entry in the research table at all; it beats the pan on the pan's own product (0.50 a barrel against 0.33) *before* the water is counted; and a tended field with no water yields 1 against 3. **The measurements drove the design, not the other way round:** inland water 0 tiles (there is none — which is why `fish` is no longer called *River* Fish), brine band 69,583 with 19,763 reachable from a shore, crust 90,795 with **zero** reachable, dead white 273,858. The first design had a hand cutting salt off the crust; there is no hand that can stand there. **And the headline claim went green on the control first time**, because the reported bug — fishing the dead salt — is LATENT rather than live: the rule offered a fish for a pan tile, and fourteen tiles of open water meant nobody could ever ask. It forces one tile of dry land beside a pan, puts the question, and puts the land back. Control answers `fish`; this build answers nothing. 8 red on the control. |
| `larder.js` | **The shelves, and where what is on them comes from.** Six claims off the three findings `_census.js` came back with. **The fabric leak was a real bug:** the crafter's cloak line read `take('fabric', 4) && take('hide')`, and `&&` short-circuits left to right — on a shelf with cloth and no hide the four fabric were already spent when the hide test failed, the `if` came out false, execution fell through the chain below and burnt a *fifth* fabric on a bandage. 1200 hideless shifts, and the premise is counted first: with no hide the cloak branch must be refused every time, so any fabric that moves past a bandage's one is the leak. **The hunter's hide** was the one output in the whole switch that never read `tradeSkill` — measured at two skills over 600 shifts each, not asserted. **The food sink runs through the day roll-over**, not a hand-called helper, because the complaint is that nothing in the *world* eats; and a second claim stages plague ON, since work stops under ruin and hunger does not. **The legendary check** was asked for and the answer was good news — none of the 12 is on any list — so it is enforced in both directions now rather than trusted to a comment. And **every listed good has a source**: shelves, the chest table (read by rolling it 4000 times, since the table is a private const) or a recipe — the claim that turns "delisted" from a hole into a decision. |
| `kinfolk.js` | **Whether the towns can carry a line at all.** The audit found **zero bodies anywhere with a spouse**, on `base.html`, `pr28.html` and `prebatch.html` alike — three walls and no door: `courtable` wants both sides `faction==='player'`, `courtableNpc` wants the initiator to be, and the bond tick filtered to player bodies so two townsfolk could never reach the 180 `marriageable` asks for. The conception loop opens `if(!a.spouse) continue`, so it had never once run for a townsperson; and the only other source of children was gated on *every* `civ` body against `def.civs + 4`, which every town already exceeded because worldgen seeds four children into each. **Nothing here calls a courtship helper by hand** — the point is that the world does it, so all six claims run the day roll-over and nothing else. **The premise is counted first:** how many unwed grown townsfolk exist at all, because "nobody wed" is only evidence if somebody could have. Then: they court, they wed, a town wedding does *not* conscript anybody into your squad (`marry`'s outsider clause is load-bearing now), they conceive, a newborn's `mother` id resolves to a real body, the trade is inherited off a real parent, and the towns hold their numbers against the designed size — with 45% attrition at sixteen left exactly where it was, because that is good texture. |
| `descents.js` | **The way further down, and whether you can find it.** Reported: *"I cannot find any entrances that lead deeper than the first layer. I'm not sure if they are invisible or if they are simply not there."* **They were there and they worked** — 14 ways down to the Deepworks and 6 to the Sump, every one open on both storeys and findable by `stairAt`. Two of the seven claims here are green on the control saying exactly that, and they are kept because they are the ones that would catch the failure the report guessed at. What they were not was FINDABLE, and the control measures it: a way down sat a median **87 tiles past the nearest hall's edge against a 62-tile draw radius** — outside the distance at which the undercroft draws its own local geometry, so it was never on screen while you stood in the hall it belonged to. The placement now rings outward from a hall of the storey ABOVE (a junction bodies walk through) rather than from the lower hall, which brings it to a median 20 tiles. **The bar comes off the renderer rather than a round number** — two earlier versions used 20 and 18 because they sounded right. And the memory half: `vis` is one bitmap for a world four storeys deep, so the sharpest claim is the NEGATIVE one — walking the waste above a shaft must not mark, remember or draw a thing three storeys under your boots. 3 red on the control. |
| `_openground.js` | **Why the camera benches cannot find anywhere to stand.** Eight of them share one copied block that hunts for a clear square to pose a body on, and it had two faults at once. The visible one: `wyrmpix.js` asks for a 21x21, then 17x17, then 13x13 of open ground across 1200 ring positions and gets **0 clear at every pad** — the rejecting filter is DECOR (754 of 1200), because one tree anywhere in the box disqualifies the box, and the bench could therefore never take a picture at all. The silent one: every copy read `self.W`, and `W` is a top-level `const`, which is **not a property of `window`** — so the width was `undefined`, `terr[iy * NaN + ix]` was `undefined`, and the water test in that block had never once run in any of the eight. This prints what each filter actually rejects at each pad, which is what turned "no open ground" into "drop the ladder to pad 4, where there are sixteen". |
| `_census.js` | **An audit of the living world: who works, who breeds, and whether the shelves move.** Three questions in one run, because running them separately would be three different worlds. Reports rather than asserts — an instrument, not a claim. Prints population by faction; townsfolk broken into grown/trade/idle/children per town; the trade mix; every town's stock at the start and end of N days with the delta; the aether-cell check (goods a vendor LISTS that no town anywhere HOLDS); caravans and carts; and the birth bookkeeping — wed, carrying, overdue, children, grown-up children with a trade. Written for the audit that found the towns demographically frozen: `node tools/_census.js game.html 12`. |
| `_mowspread.js` | **How many bows fall, and how much that number moves.** `focus.js`'s last claim is a fourteen-fight average of a seeded battle against `kills >= 2`, and no spread was ever measured for it. Five seeds per build: today 1.29-1.86 (mean 1.64), before the fire commit 0.71-2.29 (mean 1.61), `base.html` 2.21-3.00 (mean 2.63). So the bar cleared on **one seed of five** even before this session — a 20% claim wearing a pass. It also shows the thing the bar was hiding: the fight has got less lethal in both directions across the project's history, your line losing 1.3 of six where it now loses 0.3, and the bows falling at 1.64 where they fell at 2.63. |
| `_coastwake.js` | **What the coast moved, measured across seeds.** Five harnesses bisect to the commit that gave the world a coast and three of them fail on a COUNT out of worldgen. Prints the spread per seed per build for two of them: bodies in Hollowmere's yard (5,6,6,6,6 at fourteen tiles on the new world against 6,6,6,6,6 on the old — and 6 everywhere at sixteen) and strangers in the Aldercott yard (0,0,2,1,0 versus 0,0,3,0,2, **every one a drifter, on both builds** — which is what proved that one was never the coast's doing at all). |
| `_swapgap.js` | **Whether the thing being asked for is possible.** `spiral.js` puts two bodies half a tile apart and tells each to walk to a point 0.2 past the other, then asks that both orders clear. `travel` calls an order done inside 0.2 tiles. This runs the pair in isolation and reports the closest either ever gets: **0.190 and 0.139**, on this build and the pre-coast build alike. A bar one hundredth of a tile from the edge of the possible is decided by sub-tile jostling, not by pathing. |
| `_fireshot.js` | **Look at the fire.** Screenshots a body burning on open waste, because "evoke the feel of actual flame" is not a number and no claim in `flare.js` can settle it. It earns its keep twice over: the first two passes both rendered a flat white blob, and the bench is how that was seen at all. The first cause was the bench itself — it PAUSED the game, so `updateParticles` never ran and four hundred motes stacked at the spawn point at full alpha; "fixing" that by dropping the core alpha to 0.30 then made a real fire nearly invisible. Particles only live while `update` runs, which is why the probe unpauses and `flare.js` does not. |
| `_rim.js`, `_seabed.js` | **What is actually out there, and is it under the water.** `_rim.js` prints the water fraction in rings at seventeen distances from the map edge, how far inland the land starts on four rays, the dry-land total and the band split; `_seabed.js` prints the height range per ring and what fraction sits under the water plane. Between them they are how the old rim was caught reading 15-18% water when its own comment claimed a sea, and how the band heights were tuned against the plane at -0.22. Scratch benches, not claim harnesses; not in `run.js`. `node tools/_rim.js [game.html]` · `node tools/_seabed.js [file...]` |
| `_mmshot.js` | **Look at the shape.** Screenshots the 128px minimap blown up to 512 with the fog lifted, for one or more builds — the fastest way to see whether a worldgen change did what it was supposed to. The before/after pair for the coast rewrite is one hard-edged square of land against a rounded island with three visible bands. Note it presses **F9** rather than calling `toggleSeeAll` directly: the direct call ran before the fog machinery was ready and both images came back black. Not in `run.js`. `node tools/_mmshot.js game.html prev.html` |
| `rally.js` | **A watch that comes when somebody shouts, and rings the whole base.** Two reports and one cause: the only thing that could summon a watchman was its own eyes. `raiseAlarm` fired when a patroller itself saw a foe inside thirteen tiles, so a labourer cut down at the far end of a base summoned nobody — nobody had seen anything — and widening sight would have bought that at the price of a watch that walks off after everything on the horizon, which the report explicitly refuses. The trigger is BEING HIT now: `applyDamage` leaves a cry, and a watchman answers a cry that landed inside its own holding. The staging is the argument — eight sheds on a ring of radius 24 plus one in the middle, deliberately bigger than the old cap, because under the old rule a body standing on the ring links only to its two neighbours at 18.4 tiles and is handed a twelve-tile holding centred on that corner, with the middle shed not even in its cluster. That is the report reproduced, and it reads **4 of 9 buildings inside a 20-tile arc** on the control against 9 of 9 inside 29 on the fix. Spread is measured as the LARGEST UNWATCHED ARC — six bearings around the middle, biggest gap between consecutive bodies, averaged over three minutes: **239 degrees on the control, 101 on the fix.** `patrolIx`/`patrolN` had been assigned since the first version and read by nothing, so every watchman drew its own bearing and its own direction; they are dealt out now and everyone goes the same way round. One trap of its own, written into the file: the aggro claim first stood a bandit twenty tiles from a watchman and ran the beat, and went **red on the fix, correctly** — the holding is twenty-nine tiles across now, so the watch walked to within sight of it in the ordinary course of its round and then saw it with exactly the thirteen tiles it always had. That is the job working, not the range moving. It is asked twice now and neither is a simulation: `nearestEnemy` at 14.5 and at 11 (with the wild bodies inside the thirteen **hushed**, because it scores by `d / aggroWeight` and answered about a passing bandit both times), and a foe parked outside the holding for ninety seconds of beat. |
| `panels.js` | **Three panels that were in the way, and one that could not be found.** Layout claims asked of the LAYOUT — `getBoundingClientRect` and `getComputedStyle` against the rendered page, never against the strings that produced it, because a `position:sticky` defeated by an ancestor's `overflow` computes as sticky and scrolls away anyway. So the figure is checked by scrolling the pack 2241px to its end and asking where the figure actually IS. `#modalbody` is one element shared by every window in the game and keeps whatever offset the last one left, which is why the kit screen opened halfway down somebody's backpack. Then the wagon: 270px folds to 36px, the body is `display:none`, and the 36px left is the line you click to bring it back — plus [T] both ways and the choice written to `dustward_opts`, since a panel that hides and forgets is a panel you re-hide every session. Then the desk: a tab that opens `RESEARCH BENCH` from anywhere and carries the count (`RESEARCH [Y]` empty, `RESEARCH ·3 [Y]` with a project and two behind it), and the split, which is asked at `benchSides` rather than at the button — four seated scholars come out 2/2, 4/0 and 0/4 under the three settings, and the third of those is the state the old two-way toggle could not express at all. The focus survives a save, and an older save carrying only `readFirst` loads as FORMULAE ONLY. 8 red on the control. |
| `snags.js` | **Seven things reported in one breath.** They have nothing in common but the session that found them. The caravan one is the deepest: `Array.prototype.sort` SORTS IN PLACE and the night-shelter line called it on the global `towns` array, which is where `isLeader`, `warF` and `warT` all store an INDEX — so one wagon bedding down re-pointed every leader in the world, measured as **Cressa Sedge of DUSTPORT becoming Cressa Sedge of GREENREST**, and the reported symptom ("the leader of Hollowmere's name fluctuates between high warden and elder, then randomly decides on one") is the tip of it. The claim is asked as the symptom, not the mechanism, because a claim about the mechanism can be satisfied by moving the bug. The wall claim is a **whole-world invariant on purpose**: every wall tile of every walled town against `isBlocked`, because whether `raiseMountains` carves a trail through masonry is a draw — the old build leaks 4 tiles on the default seed, **8 on seed 91, and none at all on seeds 7 and 404**, so a claim written against Copperhold's four tiles would pass on two thirds of the worlds with the bug still in them. Then the harvest scan reaching three storeys down (the flat-map trap, hit for the fifth time); the notice ladder counted in RAISINGS because that is the unit the report used — **14/30/46 with steps of 14, 16, 16 on the control against 11/35/77 with steps that roughly double**; the house cloak, asked of one body built twice with the flag flipped, since skin and hair are hashed off `c.id` and two different bodies differ in a dozen colours; a dark-gifted lieutenant opening the circle it was locked out of; and The Last Rite refusing to be bought **in both states of the bench**, which is where the bug actually lived — the quest gate sat below the queue branch, so the moment anything else was running the rite grew a QUEUE button with a full price on it. Two staging faults of its own are recorded in it: the harvest job's key is `carrion`, not `harvest`; and the authored box colour is gone by the time there is a `material` — `obox` bakes into merged buckets with one white material and per-vertex colour, so reading `material.color` returns white and reading `_bc` off the proxies returns two colours out of fourteen. Both were measured before the third attempt landed on the vertex attribute. 10 red on the control. |
| `_wallseeds.js` | **Is this wall hole one world or every world?** Counts unblocked wall tiles per town across three seeds on two builds, and answers in six lines: the fix holds everywhere, the bug shows on two seeds of four. Written because `snags.js`'s wall claim would otherwise have been tuned to one map. A scratch bench, not a claim harness; not in `run.js`. `node tools/_wallseeds.js` |
| `frame.js` | **Where a frame actually goes — and the instrument this repo was missing.** Every sim measurement here had hand-timed `update()` in a tight loop, which excludes the render entirely and, when paired with per-call wrappers to find hotspots, distorts exactly what it is hunting: a wrapper costs a fraction of a microsecond, which is nothing until it is multiplied by the call count, and the call count is the thing you are looking for. `physics` at 1,102 calls a step came back as 40% of the frame and much of that was the wrapper. This drives the REAL rAF loop with the renderer running, takes a CPU sampling profile off the DevTools protocol, and attributes SELF time per function — no instrument in the path of anything hot. It is what caught a memo that was exact, removed 8,700 rule evaluations a step, and was four times slower than the rule. **Read the native rows with care:** this container has no GPU, WebGL runs on SwiftShader, and the software rasteriser is ~90% of every frame at any viewport — an artefact that does not exist on a real machine. The JS rows are faithful and are what the report separates out. `node tools/frame.js [game.html] [secs] [w] [h]` |
| `feed.js` | **Whose Attention is it, and whose feed is it.** Measured before a line was written, one game day with the player standing still and casting nothing: `noticed` went **4.6 → 37.3** and the dial went UNNOTICED → ATTENDED, of which **27.83 came from the `deep` faction and 7.50 from townsfolk** across 143 casts and 5 raisings — and **0.00 from the player**. The Attention is the Old Ones noticing *you*; it was counting every working by every caster in a world of seventeen hundred bodies, and the three new storeys added a great many casters, which is why it got worse rather than why it was wrong. The gate already existed two lines below the raise with the same fault (`mood.raise` is set only `if(c.faction === 'player')`). The feed gets the rule as the report stated it — yours always, anybody else's only where one of yours is standing and watching — and **`visAt` alone is not enough**: the fog is one flat W×H grid, so a priest four storeys down under a lit square reads as being in plain sight. Also catches a stranger's surgeon in your pack: `useBandageCharge` draws on `stash.bandage` and the healing tick is the *one* healing path in the file, so every medic in the world was spending your cloth and then announcing it had run out. **The claims come in pairs on purpose** — a feed that says nothing passes a test that only counts lines, so each asserts the noise stops *and* that your own work still speaks (the dial still moves 0 → 0.78 on six of your own workings; the stranger silent across the map still speaks at your shoulder). |
| `layers.js` | **The storey you are on, and the room you are in.** The surface minimap is the whole world — 1440 tiles in 128 pixels — where a warren chamber is two pixels and a corridor is none, so the underground layer is a **local window of 132 tiles** around your own people in the storey's own tint, the same colour the depth banner wears. It carries its own memory (`underSeen`) because the flat fog cannot say which parts of a storey you have walked; **three tones are counted exactly** — rock, open ground in the depth's tint, and ground never seen — because "it looks different" is satisfied by a blank square, and a storey nobody has visited must give away nothing (16307 of 16384 pixels unlit). The ring claim is about **all** chambers rather than about the map border: `chamber` walls its whole perimeter but the one door tile, and `wall` refuses to write outside `wx>1 && wy>1 && wx<W-1 && wy<H-1` while `deck` has no such guard — so a chamber whose ring fell on the boundary got its **floor laid and its wall silently dropped**. 1 room of 179, and it was a **vault**, nineteen tiles of its top wall absent. Two faults from building it are in its header: the flat-fog trap was walked into **twice in one sitting** (the helper written for it did not stop it), and open ground was first painted at alpha 210 so it composited with the previous frame and the storey's colour was never its own — an eye would have missed both. |
| `walk.js` | **Not walking seventeen hundred bodies to ask about three.** "Always, even standing still" is a diagnosis, not a complaint: a cost paid whether or not anything is happening scales with the ROSTER, not with the fight. Four questions about a handful of bodies were each being answered by reading all 1,757 — `player()` rebuilt 22 times a step, the bodyguard's two roster scans with `hostile()` and `dist()` predicates, `_nearSquad` asked once per body, and a full walk to move a corpse along with the two or three people carrying one. Each was re-pointed at a structure that already held the answer: a cached list, the spatial grid, the `_cold` flag computed one line above, and a list gathered in `rebuildCharGrid` alongside `warders` and `lamps`. **The claims are about EXACTNESS, not speed** — the gathered haul list is compared against the filter it replaced (0 missing, 0 extra), the grid sweep is compared against the roster scan over a hundred ward/guard pairs, and the `_cold` short-circuit is re-derived by hand on every cold body in the live world rather than asserted from the fact that one condition implies the other. A guard is then driven into stepping in, and into letting go of a ward spliced out of the roster. The millisecond figure is printed and is deliberately **not a gate**, because this repo has already shipped one speedup that did not exist. Interleaved A/B, three rounds: 13.610 / 13.383 / 13.990 ms a step before, 10.728 / 11.738 / 10.843 after. |
| `_simprof.js`, `_lines.js` | **The second and third instruments, and the first one that named a line.** `frame.js` answers "is it the sim or the draw". `_simprof.js` samples a pure `update()` loop and attributes SELF time per function, which gets as far as "`update` is 16.8% of the step" — useless on a dispatcher fourteen hundred lines long. `_lines.js` reads the `positionTicks` the same CPU sampler records **per line** and names the line: `_nearSquad(c)` at 6.1% of `update`'s own frame, the haul loop at 6.7%, the passing-jab loop at 21%. Neither puts an instrument in the path of the thing it measures. Scratch benches, not claim harnesses — they are not in `run.js`. `node tools/_lines.js [game.html] [secs] [fnName]` |
| `_home.js` | **Is this red a regression, or is it one draw?** Reproduces `lineage.js`'s homestead claim on its own — no earlier claims, no accumulated days — and prints state on every night instead of a verdict at the end: `homeAt`, `birthPlace`, the banner load, and **what the game itself logged**, since the loss branch says one line and the birth branch says another and that is the one discriminator that does not depend on reading state afterwards. Then twenty re-staged terms, so the loss rate is counted rather than guessed at. Written to answer a red that survived three re-runs and was still not a regression. Copy the `p.goto` line and append `?seed=N` to ask the same question of another stream — **any red whose failure signature has a probability in it gets re-seeded before it gets bisected.** A scratch bench, not a claim harness; not in `run.js`. `node tools/_home.js [game.html]` |
...
| `cloak.js` | What a disguise hides a risen from, and what it does not. Both halves of the report named a cause and neither one was the cause, which is why the file MEASURES both before asserting anything: the shroud has no range gate (it holds at 120 tiles) and the purple bead is not stale (it is driven off `c.shrouded`, the same flag the rules read). The town had **two eyes and only one had been told** — the alarm asks `revealedUndead` and was fooled correctly; the `walkdead` crime asked the raw `c.undead` and could not be fooled by anything, so a shrouded risen collected a bounty and 25 standing a time until the ledger passed -50, at which point `hostile` turns the town on you on REPUTATION, which does not consult disguises. The guards killed it and the spell ended with its subject, exactly as designed. Runs **one fresh world per arm**, because reputation is world state and a second staging inherits what the first did to the ledger. The control is the whole proof: over 300 ticks an empty town drifts 0, a bare risen costs -156, a shrouded one cost -217 before and 0 after — a fix that moved -217 to -40 would have looked like progress and been nothing of the kind, and one that moved the BARE number would have been a different bug. It also asserts the Gravecloth, which has said `conceal` since it was written and had never once been consulted by the law. |
| `sundered.js` | The sundered ground: what lives on it, what gets up off it, and whether you can walk through it. Three reports, and the first two are **one loop** — the site's animals died, their carcasses counted toward the fodder that calls a Corpse Cairn up, and the thing that got up ate the carcasses, so visiting a site summoned the thing that finished it off. Cutting `siteId` out of `cairnFood` breaks it at the only point that leaves the ORDINARY cairn alone, which the note asks for explicitly, and both directions are asserted: ten site carcasses add 0, ten ordinary ones still add 10, and `CAIRN_PER_BODY`/`CAIRN_MAX` are checked to be exactly what they were — that pair is the half most likely to be broken by accident while tuning the other half. The ecology is measured as a population that RECOVERS (stripped to nothing, back to eight in 50 game-hours) rather than as a spawn count, because "wiped out pretty quickly" is a claim about what happens after a fight. And the footprints are asserted as a **shape, not a disc**: every monument stops you at its middle, and 30% of a site is solid rather than 100%, because the inside of a ribcage is the best thing about a ribcage. Its own trap: the restock loop lived in `gauntTick`, not in `corpseSiteTick`, so the probe ticked the obvious function and watched an empty site stay empty for four days — the loop was moved to the function whose name claims it. **AND THE REGROWTH BAR WAS IN THE MIDDLE OF ITS OWN SPREAD.** `andItGrowsBackAfterYouClearIt` wanted a stripped site back to full in 72 game-hours and read 90. Measured across seventeen sites on four seeds: 29, 31, 31, 33, 35, 38, 39, 49, 55, 55, 66, 68, 69, 80, 90, 91, 95 — so 72 would flap on about a third of worlds, and the 96-hour loop around it was nearly as bad, since a site wanting 100 would have reported "back to 5 of 7" and read as the feature being broken rather than the clock being short. A week now (240-hour loop, 168-hour bar): clear of the ceiling, still an enormous distance from never, which is what the report — "after a fight the ground stayed empty" — was actually about. |
| `sunderpix.js` | Not a test — a camera, for the half of "should be a bit more unnerving" that no assertion can reach. Shoots all three monuments near and at play distance, coming round to the front of each (their groups are turned by `id * 2.39996`, so a fixed camera photographs whichever side the id happened to pick). It is what proved the report exact: the skull was ONE box with two black rectangles, the hand five slabs with no knuckle. Two traps of its own, both worth knowing before writing another camera: it named four HUD elements that do not exist and left the whole interface sitting over the subject, and it stitched from `file://` images, which taint a canvas so `toDataURL` throws SecurityError — read the ids out of the document, pass the bytes as data URIs. `node tools/sunderpix.js [outdir]` |
| `charm.js` | An art the body IS, rather than one it studied. **Nothing was missing** — the art, the cast path and the mana were all there (`rawMaxMana` is 15 + magic for every body alive), and `innateArt`, `spellsFor`, `castReady` and `beginCast` had all been taught about innate lines when the mimics were written. Three other places asked `if(c.gift)` as shorthand for "is this a caster", and all three are the report: the character panel draws the entire spell row inside that test, so a giftless succubus had no button; the C and V hotkeys; and the NPC caster branch. So the assertions are about REACHABILITY, driven through the panel's own button and the real tick, because "it works if you call `castCharm` yourself" was true on the build before and is not what anybody reported. The clash the note asks about turned out not to be duplication (asserted: it qualifies two ways and appears once) but ORDER — left in declaration order, C and V land on different spells for two succubi with the same art. Two traps: the dust attunement was the wrong gift to test the UI with, since charm is itself a dust art and the only other one needs research, so there was no learned button to tell the innate one apart from; and the first version drove `ai` eighty times and reported she never used it, when `ai` ends around line 9410 and returns on `if(c.target) return` — all combat is in `physics`. The tell was that it called **nothing at all**. |
| `unstuck.js` | Whether a body that is getting nowhere tries something else. `roads.js` samples the world's own caravans and reports zero stalls over nine days, which is exactly why this file does not do that: the fault is RARE, and a sampler watching the ordinary case keeps saying everything is fine. So every staging is a body put deliberately in the bad spot — pinned outside a town ring, sealed into a pocket, standing on solid rock — and the question is not whether it is stuck but whether it is still stuck a minute later. The old loop had **no escalation in it**: `findPath` fails, the body walks a straight line into the rock, `pathFail` suppresses the re-flood for four seconds, and then it asks the identical question from the identical tile. The counter (`unstuckN`) exists so this file cannot pass on a build with no fallback simply because every staging happened to be solvable. The assertion that matters most is the one that must NOT change — a 40-tile walk across open country escalates zero times, because an escalation that fired on ordinary travel would send every unit in the game on detours it did not need. The town-wall staging passes on **both** builds and says so: the existing "if the truncated leg fails, ask for the real goal" already covered that one. **And its first version broke three fighting harnesses**, which is the most useful thing it taught: inside a few tiles of the goal a body stops making progress all the time and none of it is a pathing failure — it is at the target and the target is surrounded, or its own line is in the doorway. Sending that body on a perpendicular detour is a retreat, not a rescue, and press.js went from twelve bodies on six sides to four while the Sixfold's stamp stopped finding a crowd to land on. `STUCK_NEAR` keeps the fallback out of the last four tiles. |
| `deepaim.js` | Whether an order given underground goes where you pointed. `cave.js` already asserts that an order aimed at a tile they are STANDING ON lands on it, and it passes — this is about the click that misses, and about everything else in the chain. **Three bugs, one root: nothing in the right-click chain asked which storey you were on.** `wx, wy` came from `screenToWorld`, which marches the terrain and stops at the mountain overhead, and all twenty-one `bodyHit` tests below inherited it — so no body in a cave was ever under the cursor, and when `storeyHit` found no carved floor the destination fell through to that surface point at FLOOR ZERO and the squad set off for a shaft to climb out. The third only became reachable once the first two were fixed: `bodyGap` is horizontal, so a creature one floor up at the same tile is under the cursor, and a move order aimed at empty floor came back as an attack on a Carrion Maw one storey above. Its own two traps: the first version aimed its "miss" three tiles out at rock that happened to touch the hall, watched `storeyHit`'s one-tile forgiveness catch it correctly, and proved nothing — a genuine miss needs rock with no carved tile within two; and its camera settle broke on a single quiet frame, which is not wrong so much as INTERMITTENTLY wrong, reporting the squadmate click working on one run and failing on the next because the click was aimed through one camera and measured through another. Five consecutive still frames, and three clean runs before it was believed. And a SECOND intermittency was hiding behind that one, which is why the file went red again the moment it was run a hundred times: the chain has a deliberate fast path in front of the squadmate menu — a WOUNDED squadmate sends the ablest medic and returns, opening nothing — and something in the dark bites the staged mate during the seconds the camera spends settling. Correct behaviour and the failure symptom are the same absence. The mate is now healed on the frame before the click, and the line says what it healed. |
| `discourse.js` | What the world has heard, and what it says to you about it. **Fame is two numbers and that is the design**: `known` only rises, because a story does not unhappen, and `repute` moves both ways — the BAND comes off known and only the WORD comes off repute, so famous-and-loathed is a place on the scale rather than a cancellation. Asserted by holding known at 70 and flipping repute: same rung, "notorious" against "renowned". The first version of that test ran twelve conquests against twelve towns saved and compared bands, but a conquest is worth 8 known and a saved town 7, so the arms landed on different RUNGS and it reported a design failure that was really a difference in deed weights. The dialogue half is asserted through the **rendered buttons**, never off the tree data — every gated option existed in the table before any of it was wired to `talkTo`, so "the option exists" proves nothing; what the report asks is that the right person sees it. Its own care: the fame sections cannot degrade (there is no earlier version of a thing that does not exist) but the dialogue sections can and do, so the negative control reports no modal and empty menus rather than one ReferenceError.  Extended for the checks: every option in the trees was a GATE — you have the thing or you do not and the answer never changes — so a `check:{dc}` line you may ATTEMPT, with its odds on the button, is the half that makes charisma a build rather than a discount coupon. Asserts that the odds are shown, that the tongue moves them, and that a person REMEMBERS being worked on, without which a failed persuasion is a free reroll off the back of closing the window. Also that the other necromancer answers: `npcNecro` sat in the exclusion list beside the Coil cultist, which is right for the Coil (one strange sentence and a turned back is the characterisation) and exactly wrong for the only other person on the map doing the player's own work. |
| `anchored.js` | What a binding circle can carry, and what the cheapest dead are worth. Two halves of one economy, and **both prices have to come off or it is half a feature** — the slot off `risenCap` and the permanent bite `bindStrain` takes out of the master's mana — so every assertion reads both numbers on the same bodies. The **cap per circle** is the assertion that matters most and it is not in the original note: without one a single ring holds an unlimited host and the ceiling stops existing, which is not "garrisons do not tax the caster" but "there is no cost". Two traps of its own: the walk-out test emptied a slot in a FULL ring and one of the three waiting outside was promoted into it, so the load was correctly unchanged and the probe called that a failure — it wants an under-full ring; and the Longdead's stats came out 2 or 5 across identical calls, which turned out to be `makeChar` rolling a bloodline and adding its flat gifts on top of the numbers the caller handed it. That one was a game fix, not a probe fix. |
...
- **A speciality good has its scarcity pinned at the ceiling, so its multipliers stack on top of
  that.** Harbourblack is stocked in exactly one town by design, which means `scarcity` is at its
  maximum in the other six *permanently* — and the town multipliers I picked by eye then multiplied
  that, giving a 13× spread against ordinary rum's 4.6× and a money printer disguised as flavour.
  Price a new good by asserting its spread against an existing good's, not against a number that
  looked reasonable in isolation.
- **A fight the probe can lose is not a measurement of what happens when it wins.** `pit.js`
  took the three starting characters into the arena's easiest card, lost, and reported
  "NOTHING WAS PAID OUT" — which is a true statement about a party that lost and says nothing
  whatever about the payout path. Two debtors beating three fresh recruits is the pit working.
  Stage the outcome you are measuring, and stage the other one separately, rather than letting
  a dice roll decide which branch of the code gets walked.
- **Watching less than one cycle of a thing measures the observer.** Hollowmere's gravekeepers
  scan for a body every three GAME seconds, and a live render was watched for forty WALL seconds
  — about one scan — which reported the rite dead. Driven through `update()` the same build
  raised all six paupers inside a minute. Before concluding a periodic system is broken, find
  its period and watch several of them.
- **A suggestion loses to a rule that runs every tick.** The same keepers' scan set a
  `moveTarget` toward a claimed corpse; the guard leash overwrote it on the next tick, because a
  posted body is dragged back the moment it strays 1.5 tiles from its flagstone. Whether the
  keeper ever arrived came down to whether `travel` happened to overshoot the post in the right
...
- **A WebGL canvas is empty by the time you get around to reading it.** `kitdoll.js` copied the
  portrait canvas with `drawImage` and counted zero lit pixels off a figure that was plainly
  drawing in a screenshot taken the same second: the drawing buffer is cleared once the browser
  composites it, and `preserveDrawingBuffer` is off by default. Render and copy in the same task
  with nothing in between, or screenshot it from outside the page.
...
- **Anything going through `mk()` must be Lambert or Standard.** `mk` registers its material in
  `e.mats`, and `syncChars` walks that list every frame writing `m.emissive.setRGB(...)` and
  `m.opacity`. A `MeshBasicMaterial` has no `emissive`, so one unlit highlight in an eye socket
  threw inside the render's try/catch — logs once, then stops repainting. Second frozen render
  of the session from one unguarded property, and both stacks were only reachable by reading the
  error **argument** out of `console.error('frame error:', err)`.
- **A `guard` post fights whatever else sets `moveTarget`.** `physics` enforces one whenever a
  body has nothing else to do, and "nothing else to do" is exactly what a deliberate holding
  pattern looks like from outside. The Kept, posted to their own hall centre, walked calmly onto
  the player standing in it — staged at ten tiles, measured closing to three. A body with its own
  movement rule needs its own field, not the shared standing order.
- **`c.dir` is written by things and read by nothing.** The renderer's facing chain is "face what
  you are fighting, else face the way you are walking, else keep what you had", with no branch
  for a body that is standing still and looking. If you want something to watch, add the branch.
- **One flag, two meanings — for the third time.** `c.wisp` means "draw this as a floating
  shell"; it is not the same claim as "this gives off light". The shoallings float, so every Kept
  in the world read its own shoal as a lamp and spent its life retreating from it: 70 lamps in a
  world that had one. Same shape as `e.servitor` and as `raised`. When a flag is set by three
  rigs and named after one of them, that is the bug.
- **Geometry that follows the thing you are measuring is not a measurement.** `deep.js` counted
  solid tiles within six of the Stonewake, before and after — and the Stonewake moves, so it
  reported the rock going **up** while the thing was cutting through it. It had simply walked
  somewhere stonier. Measure the corridor, not the neighbourhood.
- **A worldgen seeder that runs before line 7348 must not name `baseBlocked`.** It is a `const`
  captured from `blocked` at that line, so touching it earlier throws
  `Cannot access 'baseBlocked' before initialization` — **inside an IIFE**, which swallows the
  rest of the seeder silently. The Aldercott house shipped its first build with no upstairs, no
  stairs and no door, and the only symptom was a missing key on an object. Anything added to
  `blocked` before the capture is in `baseBlocked` by construction, so the call is never needed.
- **A refusal a walk order can go round is not a door.** Bellowes turned you away in dialogue
  while the doorway stood open in `blocked`. `seats.js` proves the fix by flooding the yard for a
  path to the interior, not by asking the conversation what it thinks — the same distinction the
  warren seal was built on, and the same one `jail.js` records about reaching past the door the
  player has to use.
- **A "deprioritise" that pushes past the search radius is a "hide".** `nearestEnemy` seeds its
  best score with the radius itself, so a body scoring worse than that is never found at all.
  The Gaunt's Mantle written as a very small `aggroWeight` would have read correctly in every
  test with two targets and made a gaunt stand still in a corridor with one. Hold the
  deprioritised body in a second slot and return it when the sweep found nobody else.
- **`snapshot()` holds live references, so a probe must serialise before it mutates.**
  `parts: c.parts` is the object itself — fine in the game, where the snapshot goes straight to
  `JSON.stringify`. A harness that takes the snapshot and then edits the body to prove the
  restore works is editing the snapshot too, and measures a save that never held the value.
- **One probe per reading is one creature per reading.** `makeChar` rolls a subrace, and several
  subraces carry a `vuln` row that `mitigate` applies after the armour — so the Eleventh Coat's
  four damage types, measured on four fresh bodies, came back with a spread that belonged to the
  bodies. Same body, one thing different, or it is not a comparison.
- **A rule read in five places is rarely the same rule five times.** `consecratedAt` gates
  raising (three sites) *and* what holy ground does to the dead standing on it (two). Hooking
  the function would have made a coat about necromancy quietly grant the whole host immunity to
  shrines. Hook a predicate at the sites that are the rule you mean.
- **Ask who a change newly blinded, not just who it was meant to.** Dropping `caveDweller` from
  `seesInDark` was correct — it means "this body's AI lives in this warren", and worldgen puts it
  on the bandits sheltering there too — and it also took the redoubt garrison with it, 28 bodies
...
- **A doused light is not an absent light.** `dark.js` zeroed every Pitch Post's fuel between
  ...
  woodpile before the post under test got a look at it. The teardown has to remove the thing,
  not put it in the state the mechanism is designed to recover from.
- **A three.js light with `visible = false` is a light the scene does not have.** Objects marked
  invisible are skipped when the renderer walks the scene, so toggling one rebuilds every Lambert
  shader in the world — and the frame that happens on is the frame you carry a torch into a dark
  room. The three torch lights are added once and dimmed to zero instead; intensity is a uniform.
- **A blueprint had no storey, so building underground silently built somewhere else.**
  `isBlocked(x, y)` defaults to floor zero and `placeStructure` only ever wrote a floor for
  emplacements, so staking a bin while the camera was down the shaft tested surface rock, raised
  it on the surface, and drew it twenty-two units over your head. Nothing errored. When a
  coordinate system has three floors and a record carries two of them, the missing one is not a
  default — it is a bug waiting for someone to walk downstairs.
- **A frozen render with a live sim is the hardest failure in this file to read.** The demilich's
  first build produced two screenshots four minutes apart that were *identical byte for byte*
  while the camera, the party and the clock were all demonstrably moving underneath. `!c.beast`
  was the whole test for "this rig has arms" — true for exactly as long as every non-beast in the
  game was a person — so the limb pass reached through `e.armL` into `.visible` on a rig with no
  arms and threw **inside the frame's try/catch**, which logs once, keeps the world turning, and
  stops repainting. If a screenshot will not change, suspect the paint before the sim. And the
  stack is only reachable by reading the error ARGUMENT out of `console.error('frame error:', err)`
  — a probe that captures `msg.text()` alone sees a message with no line number in it.
- **Adding one body at worldgen re-rolls the rest of worldgen — unless you pocket the stream.**
  `makeChar` spends numbers off the world stream whatever it is handed, so `spawnDemilich` would
  have moved the Dustport Archivist, three bone golems, four keepers, the arena, the orchard, the
  saints, the lanterns and the mine. `seed` is a plain global: borrow it, spawn, put it back in a
  `finally`. **Measured, and this is the only proof worth having:** a fingerprint of 1,087 other
  bodies with names, tiles, sex, race, line, gift and conviction, plus seven towns and eight
  ruins, hashes to the same number on both builds. This is the cheapest fix for the most
  expensive recurring mistake in this repo, and it should be the first thing tried next time.
- **Geometry is not behaviour.** `albedo.js` first checked that two towns were *nearer* Saltmere's
  ...
  The distances were never the question. Burn somebody from that town and see where the pole goes.
- **You cannot dig into ground that is already at the floor.** Copperhold's pit was cut four
  units into terrain sitting near sea level and `groundY` clamps at −0.1, so it measured **0.0
  units deep** while every line of the digging code was doing exactly what it said. The fix was
  upstream of the fix: raise the seat onto a plateau first, then cut. Same ordering lesson the
  mountain trails learned — anything carved before the terrain is finished gets filled back in by
  whatever runs after it.
- **When a probe correctly catches your own deliberate change, split the rule rather than
  relaxing it.** `world.js` asserts "not one tree, boulder or outcrop inside any town wall" and
  went red on the sixteen tiles of copper in Copperhold's working. Adding a skip for the pit and
  leaving it there would let the exemption grow silently, so the claim became two: nothing wild
  inside any wall, *and* something there had better be in the seven tiles of the pit.
- **"Where did it stop" and "did it reach you" are different questions, and a ward answers the
  second.** `lantern.js` measured a gaunt's final distance, which works when the light is on a
  lamp the target stands under (a turned gaunt is pushed out past where it started) and fails
  when the light is on the target (the gaunt closes to the edge of the radius and hovers there,
  correctly, forever). It read 9.0 to 7.5 and reported the charm dead about a charm doing exactly
  its job. Track the minimum over the walk.
- **A claim both builds pass is not measuring your change.** `salt.js` first tested that the raise
  rite refuses in the middle of Saltmere — and it refused on the build before too, because every
  town has a shrine and 19 tiles covers a town square. The whole point of the change was the
  ground *outside* the wall, where an ordinary stone does not reach and this one does. Put the
  probe where the two builds disagree, and run the control early enough to find out that they
  don't.
- **A negative control that dies of a `ReferenceError` proves only that the symbol is new.**
  `mere.js` referenced `hollowmereBurials`, `YARD_CAP` and the two risen caps directly, and the
  build before this work took the whole file down on the first one — no claims reported, red for
  the wrong reason. Reach for anything new through `typeof` and the old build goes red *with
  readings*, which is the entire point of running it.
- **A per-item loop cannot see between the items.** `seedWarrens` raised each warren's wall ring
  and then cut its approaches, which is correct within one warren and blind between them: warren
  N+1's approach was cut after warren N's ring was up and after N's drawn-wall list had been
  filtered, so it punched clean through N's ring and left a tile that was walkable and still
  painted as rock. `warrens.js` (33 of 120 chambers reachable with every door SHUT) and `under.js`
  (4,108 tiles solid against 3,496 drawn) were the same hole counted from two sides. The fix is
  a shape, not an arithmetic patch: place everything, then route around what exists, then raise
  once. **And it read 0 of 112 on the seed the work was done on** — the fault was always there and
  the random stream decided whether any two warrens happened to overlap.
- **A helper that "opens a tile" opens a door.** `U.put` bails on `decks.has(k) && !blocked.has(k)`,
  which a SHUT door fails — it is decked *and* blocked — so it fell through and deleted the block
  from `blocked` and `baseBlocked` both. The door object still said `open: false`, so the tile
  stayed walkable until a reload put it back: `under.js` saw twelve more rock tiles AFTER a save
  than before, which is a strange enough number to chase and the only sign of it. Anything that
  carves has to treat a door as unroutable.
- **An assertion can pass because the subject is BROKEN, and invert when you fix it.**
  `beasts.js` read `carried(cart) > 0` at the end of a Gravecart's round, and it only ever passed
  because the cart could not finish: it aimed at the yard centre, let go at 2.2 tiles, and a 3x3
  yard reaches 2.0, so it circled the racks holding the load forever. The delivery fix landed and
  the probe reported "THE CART NEVER PICKED ANYTHING UP" about eight bodies neatly racked. Measure
  the ROUND — the fullest the bed ever got, and where the bodies ended up — not a snapshot of a
  moment whose meaning depends on the bug.
- **Moving the camera moves what the game can SEE.** `touch.js` panned onto deliberately empty
  waste to find open ground and left the ordering unit sixty tiles behind; the attack assertion
  after it stands a bandit on that dirt, and `attackMoveMode` is gated on `visAt(foe) === 2` —
  current sight, not memory. The target was in fog, the handler correctly refused to issue an
  order, and the probe called the touch layer dead. Stage the whole scene, not the half of it you
  are looking at.
- **Compare against the body you got, not the options you asked for.** `steeped.js` passed its
  staging opts to `makeChar` and then asserted against those opts — but `makeChar` draws a random
  race and subrace whose modifiers move the stats, so `magic 20 of 20` passed while `atk 12 of 9`
  failed on the same character. Pin the line, then capture what you were handed.
- **A probe that stages "somewhere else" has to check where that is.** `kin2.js` staged its
  homeless couple three tiles from the player's start, which sits beside GREENREST — `birthPlace`
  found an inn bed and the homeless case was never once exercised. It now scans for waste ninety
  tiles from any town and prints the distance to the nearest bed (193) in its own output, so the
  next person can see the staging was real.
- **Ask the question the thing you are about to call is going to ask.** Hollowmere's keeper tick
  gated on `risenCount >= 7` and then called `castRaise`, which gates on `risenLoad + 1 >
  risenCap` — a count against a flat number, then a weighted sum against a computed one. Both
  were correct; they were not the same question. A keeper who reached the second ceiling passed
  the first gate, walked to a corpse, failed the rite, dropped the claim, and came back for the
  same corpse three seconds later forever. The symptom that got reported was the LOG. Two gates
  in front of one action have to agree, or the cheaper one is just a way to keep failing the
  expensive one on a timer.
- **A fixture that sets the wrong field fails open, and reads as a pass.** Two `quiet.js` claims
  staged "this caster's binding is full" with `k._bindW = 99`. `bindStrain` reads `_bindW`;
  `risenLoad` walks `chars` and sums `bindWeight` over the bodies whose master this is, and has
  never heard of it. One claim printed `wouldRite: false` in its own failure text while asserting
  a caster past the ceiling; the other watched the rite SUCCEED and took the crime line that
  follows as proof the feed still spoke. Neither was testing a full binding. When a claim needs a
  system in a state, put it in that state through the thing that reads it.
- **A claim about silence must name what it is listening for.** Counting every line the feed took
  during twelve staged NPC raisings came back with two, and both were regard tells — a hand of
  yours reacting to watching somebody else's necromancy, which is exactly the kind of line that
  must keep arriving. A blanket "the log stayed empty" would have been fixed by deleting
  something the player wants. Match the vocabulary of the path under test, and REPORT the rest in
  the pass line rather than discarding it, so a line worded some way the list does not know still
  shows up instead of vanishing behind a green.
- **A dead branch rots, and it rots silently.** `stockRoom`'s shallow-warren table was written
  correct and could not be reached for a year — two separate faults saw to that. The day both
  were fixed it ran and produced nothing: it calls `makeChar`, which builds a body and does not
  add it to `chars`, while the sibling branches call `spawnGaunt`, which does. The interface had
  grown a step underneath code nothing executed. Twenty-four warrens stocked a hundred and one
  chambers with bandits that were named, armed and handed a torch, and every count in the game
  stayed exactly the same. When you revive dead code, do not assume the parts of it you are not
  changing still work — nothing has run them either.
- **Fixing the number without fixing the order changes nothing.** `cv.menace` was both the wrong
  scalar AND assigned after the thing that reads it. Correcting the scalar alone moves every
  warren from one wrong table to a different wrong table; correcting the order alone hands the
  right table the wrong number. Measured after fixing only the value: warrens correctly spread
  24/15/6 across three storeys, and still not one bandit in the world. Two faults on one line
  will each hide the other's fix.
- **The average is not the promise.** A per-body roll at one-in-three delivers one-in-three
  across a world and says nothing about any particular room. 90 torches among 276 bandits was
  the designed rate and left 83 men in chambers where the roll had come up empty every time.
  When the design sentence names a unit — "lights the room for the rest of the band" — that unit
  is what has to be guaranteed, and the guarantee is usually cheaper than the roll.
- **Two derivations of one list will drift; the fix is having one.** Worldgen filtered the cave
  wall list the ring-builder pushed, and `restore` derived it fresh from the rooms. They agreed
  for a year and then disagreed in both directions at once. Chasing the two into agreement is not
  a fix — make the second caller ask the first one's function.
- **A greedy stepper cannot get out of a pocket.** The connectivity repair walked toward its
  target and sidestepped once when blocked, which is fine on open ground and hopeless inside a
  ring: escaping a pocket means moving away from the target for a while, and that is exactly what
  a greedy walk will not do. It left one warren sealed with all four chambers inside — and the
  tell that it was a mechanism rather than noise was that the four failures were ONE PLACE. When
  a handful of failures share an address, you have one bug, not a handful.
- **Test the obvious performance theory before acting on it.** Three storeys in one x/y space
  looked certain to be stacking the `charGrid` buckets and making every proximity query pay for
  bodies four storeys away. Measured: average bucket 1.9 and biggest 14 on both builds, because
  the three lattices sit at different x/y and the bodies do not stack. The cost was simply more
  bodies. An hour of refactoring the grid would have bought nothing and been very easy to
  believe in.
- **A flag named for one question must not be made to answer a second one.** `_lod` meant
  "nobody is watching them overlap" and had exactly one reader: the pair-skip in `separate`.
  Folding a new "nobody is on this storey" idea into it read as obviously right and silently
  broke every fight underground, because `separate` is also where `_contacts`, `_holders` and
  `_pinNow` are written. Two bodies acquired targets, wound up, swung, and took not one point off
  each other in five thousand steps. If the new condition is a different question, give it a
  different flag and let each reader say which one it wants.
- **Correct the premise before borrowing from it.** Kenshi is not smooth — it is known for
  late-game slowdown and framerate collapse in big fights. What it is good at is making a huge
  world feel continuous while simulating almost none of it. Taking "how do they keep FPS high"
  at face value would have sent the work at frame cost; taking the real achievement sent it at
  *how much is being simulated*, which is where the 40% was.
- **A cache in front of a cheap function is a tax, and a call count is not a cost.** `hostile`
  was the most-called function in the game — 8,761 times a step through ninety-six lines of
  faction law — so it looked like the obvious target. It was memoised on a key carrying every
  field the rule reads, verified exact against the rule on fifteen thousand real pairs, and it
  took those 8,761 evaluations down to 44. **It was four times slower.** The rule's first line is
  `if(a.faction === b.faction) return false;` — most calls die on one string compare, a memo
  cannot reduce the number of CALLS, and the guard needed to keep the table honest was twenty
  times the work of the early-out it stood in front of. Measure what the function *costs* before
  assuming that calling it a lot is the problem.
- **If you do cache, a key must carry every input and the proof is a comparison, not an
  argument.** While it existed, the enmity table was checked on fifteen thousand real pairs
  against the unmemoised rule — with the count of HOSTILE pairs asserted alongside, because a
  comparison where every answer is `false` agrees perfectly and proves nothing. That part was
  right, and it is why removing the cache was a judgement about speed rather than a retreat from
  a bug.
- **Your instrument is part of the experiment.** Two of this session's wrong turns were the
  measuring apparatus, not the code: a per-call wrapper inflated whatever was called most and
  pointed the work at `physics`; and a "35% speedup" was two numbers taken at different times
  with different scripts, which a controlled interleaved A/B on an idle machine flattened to
  nothing. Wrapping is exact for something called once a frame and poison for something called
  ten thousand times. When the answer matters, sample the real loop (`tools/frame.js`).
...
- **A probe that commands an NPC through a field its own AI owns is measuring the AI.**
  ...
  the whole time. The body's ai cleared the field within the second, as it would at any clock
  rate. Measure the property the change actually promises (ground covered per second, cold
  against warm) rather than obedience to an order the subject never accepted.
- **One pair is not a fight.** A combat claim staged a single duel and asked whether either
  died. The same duel on a control floor came back at 72, 85 and 86 blood on three consecutive
  runs and never killed anybody, so the red line it printed about the experimental floor was
  reporting variance. Twelve bodies and a total, A/B at the same spot with one variable changed.
