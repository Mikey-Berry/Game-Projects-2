...
(async () => {
  ...
  if (process.env.DUSTWARD_ROBED) await p.evaluate(() => { window.__robed = true; });
  ...
  const info = await p.evaluate(() => {
    ...
    /* DUSTWARD_ROBED=1 photographs and checks the OTHER figure this switch selects, so the
       same claims cover both rather than only whichever one happens to be the default. It is
       set before the body is built, because `charMeshes` caches past it. The default is THE
       RELIQUARY now and the flag names the robed one; when the flag named the native one it
       was silently a no-op the day the default flipped, which is the failure mode of every
       env var that names a value instead of a deviation. */
    if (window.__robed) NATIVE_LICH = false;
    chars.push(c);
    window.__id = c.id;
  ...
  const R = await p.evaluate(() => {
    ...
    {
      /* AND IT IS THE HEAD BONE, NOT `e.hood`, WHEN THERE IS NO HOOD. `NATIVE_LICH` swaps the
